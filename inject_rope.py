...
logger = logging.getLogger("inject_rope")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas RoPE...")
    ...
    md_path = "AUTODIDACT_ROPE.md"
    with open(md_path, encoding="utf-8") as f:
        content = f.read()

    pattern = re.compile(r'-\s*\*\*(ROPE-\d{3})\*\*:\s*`?([^`]+)`?\s*-\s*(.*)')
    matches = pattern.findall(content)

    if not matches:
        logger.error("No matches found in AUTODIDACT_ROPE.md")
        return

    agent_id = 99
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "ROPE_ROOT", 0.0)
    ...
    for i, match in enumerate(matches):
        ...
        logger.info(f"Ingested {p_id}: {event_hash}")
        ultramap.update_agent_position(agent_id, (i + 1) * 1.0, 0.0, 0.0, "ROPE_LEAF", 0.1)
