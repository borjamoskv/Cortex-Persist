...
async def run_gossip_benchmark(node_count=6):
    ...
    try:
        ...
        print("\n## Convergence Summary")
        if fully_converged:
            print(f"✅ Success: All {node_count} nodes converged to version {target_version} and completed routing tables.")
            print(f"- Convergence Latency: {duration_ms:.2f} ms")
        else:
            print("❌ Failure: Did not converge within timeout.")
            for i, node in enumerate(nodes):
                print(f"  Node {i} state: version={node.known_state['version']}, peer_count={len(node.peers)}")
