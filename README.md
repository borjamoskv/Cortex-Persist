...
pip install "cortex-persist[knowledge]"       # Chroma knowledge sync
pip install "cortex-persist[api,mcp,daemon]"  # MCP server + REST API
pip install "cortex-persist[cloud]"           # PostgreSQL + Redis + Qdrant
pip install "cortex-persist[acceleration]"    # Rust-FFI core (~390k agents/sec)
```

**Requirements:** Python 3.10+. Zero external daemons for local mode.
