...
logger = logging.getLogger("inject_causalidad_100")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas...")
    ...
    md_path = "AUTODIDACT_CAUSALIDAD_100.md"
    with open(md_path, encoding="utf-8") as f:
        content = f.read()

    pattern = re.compile(r'-\s*\*\*(CAUSA-\d{3})\*\*:\s*`?([^`]+)`?\s*-\s*(.*)')
    matches = pattern.findall(content)

    agent_id = 100
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "CAUSALIDAD_100_ROOT", 0.0)
    ...
    for i, match in enumerate(matches):
        ...
        ultramap.update_agent_position(agent_id, (i + 1) * 1.0, 0.0, 0.0, "CAUSALIDAD_100_LEAF", 0.1)
    
    await asyncio.sleep(1.0)
    logger.info(f"Se inyectaron {len(matches)} primitivas deterministas.")
