...
from .verification import solve_challenge_async
...
class MoltbookClient:
    ...
    def __init__(
        ...
        self._vault = IdentityVault()
        self._agent_name = agent_name
        
        # Identity Priority: 1. Manual API Key, 2. Vault by Name, 3. Legacy JSON, 4. ENV
        self._api_key = api_key
        if not self._api_key and self._agent_name:
            identity = self._vault.get_identity(self._agent_name)
            if identity:
                self._api_key = identity["api_key"]
        
        if not self._api_key:
            self._api_key = self._try_load_api_key()
    ...
    async def _request(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                # AIROS-Ω: Automatic Cognitive Immunity (Verification)
                if (
                    data_resp.get("verification_required")
                    and "verification" in data_resp
                    and path != "/verify"
                ):
                    verification = data_resp["verification"]
                    challenge = verification.get("challenge_text")
                    code = verification.get("verification_code")

                    if challenge and code:
                        logger.info("🧩 [RADAR-Ω] Challenge detected. Invoking AETHER-1...")
                        answer = await solve_challenge_async(challenge, llm=self._llm)
                        if answer:
                            logger.info("✅ [APOTHEOSIS] Challenge solved: %s. Submitting...", answer)
                            verify_resp = await self.submit_verification(code, answer)
                            # Merge verification info into original response for caller info
                            data_resp["verification_result"] = verify_resp
                        else:
                            logger.warning("❌ [RADAR-Ω] Could not solve challenge: %s", challenge)

                return data_resp

            except (httpx.ConnectError, httpx.ConnectTimeout, httpx.ReadTimeout) as e:
                # EFECTO-3: Retry connection errors with exponential backoff
                if attempt >= max_retries:
                    logger.error("Moltbook API connection failed after %d retries: %s", max_retries, e)
                    raise
                
                wait_time = (2 ** attempt) + random.uniform(0, 1)
                logger.warning("🛡️ [RESILIENCE] Connection error [%s]. Retrying in %.1fs... (%d/%d)", 
                               e.__class__.__name__, wait_time, attempt + 1, max_retries)
    ...
    async def register(self, name: str, description: str = "", email: str | None = None) -> dict[str, Any]:
        """Register a new agent. No auth required."""
        payload = {"name": name, "description": description or f"CORTEX Dynamic Agent: {name}"}
        if email:
            payload["email"] = email
        ...
        if api_key:
            ...
            # Multi-layer persistence (Vault + Legacy JSON)
            self._vault.store_identity(name, api_key, email=email)
    ...
    async def setup_owner_email(self, email: str) -> dict[str, Any]:
        """Set up an owner email for the agent to enable human login."""
    ...
    async def get_profile(self, agent_name: str) -> dict[str, Any]:
        ...
        return await self._request("GET", f"/agents/{agent_name}/profile")
    ...
    async def create_submolt(self, name: str, description: str) -> dict[str, Any]:
        """Create a new submolt."""
        return await self._request(
            "POST", "/submolts", data={"name": name, "description": description}
        )
