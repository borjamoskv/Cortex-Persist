...
class SearchIndex(Protocol):
    def upsert_fact(
        self, tenant_id: str, fact_id: int, vector: list[float], payload: dict[str, Any]
    ) -> None: ...
    def search_cross_tenant(
...
class MockQdrantIndex:
    ...
    def upsert_fact(
        ...
        self.store = [
            ...
            if not (item["tenant_id"] == tenant_id and item["fact_id"] == fact_id)
        ]
        self.store.append(
            {"tenant_id": tenant_id, "fact_id": fact_id, "vector": vector, "payload": payload}
        )
...
class FederationRouter:
    ...
    def route_write(
        self, tenant_id: str, fact_id: int, fact_content: str, vector: list[float]
    ) -> None:
        ...
        self.index.upsert_fact(
            tenant_id=tenant_id, fact_id=fact_id, vector=vector, payload={"content": fact_content}
        )
    ...
    def route_cross_query(self, query_vector: list[float], limit: int = 10) -> list[dict[str, Any]]:
        ...
        if self.cross_queries_count > 50:
            logger.warning(
                "THRESHOLD EXCEEDED: cross_tenant_queries_per_second > 50. Consider scaling out."
            )
