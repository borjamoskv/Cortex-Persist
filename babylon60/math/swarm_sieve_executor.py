...
def execute_sieve_chunk(start_n: int, count: int, worker_id: int):
    ...
    try:
        for i in range(count):
            ...
            logger.info(
                f"[Worker-{worker_id}] Inyectado [N={n}] -> MILLENNIUM-RIEMANN-{n:06d} | Hash: {node.hash} | Taint: {node.taint_signature[:20]}..."
            )
...
def orchestrate_swarm(start_n: int, total_count: int, num_workers: int):
    logger.info(
        f"Levantando Ouroboros Swarm V3. Total: {total_count} ceros, Workers: {num_workers}"
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Uso: python -m cortex.math.swarm_sieve_executor <start_n> <total_count> [num_workers=10]"
        )
        sys.exit(1)
