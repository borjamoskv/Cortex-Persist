...
logger = logging.getLogger("cortex.engine.causal.epistemic_physics")
...
class SemanticParticle:
    def __init__(self, claim: Claim):
        ...
        if claim.evidence_list:
            self.mass = max(0.01, sum(e.confidence for e in claim.evidence_list) / len(claim.evidence_list))
        else:
            self.mass = 0.5
