...
class AutopoieticAgent:
    ...
    async def evolve_cycle(self) -> dict[str, Any]:
        ...
        last_fitness_record = (
            self._genome.lineage.fitness_history[-1]
            if self._genome.lineage.fitness_history
            else None
        )
        failure_trace = (
            last_fitness_record.metadata.get("failure_trace") if last_fitness_record else None
        )
