...
def encode_state(state: SystemState, embed_fn: Callable = fast_pseudo_embed) -> np.ndarray:
    payload = " | ".join(
        [state.git_diff, state.ast_hash, " ".join(state.active_tasks), " ".join(state.error_log)]
    )
...
def embedding_risk(
    state_vec: np.ndarray, task_vec: np.ndarray, failure_centroids: list[np.ndarray]
) -> float:
    ...
    min_dist = min(1 - cosine(task_state, c) for c in failure_centroids)
...
def efel_priority(
    ...
    embed_fn: Callable = fast_pseudo_embed,
) -> float:
