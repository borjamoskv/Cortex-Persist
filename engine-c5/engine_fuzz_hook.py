                        ...
                        try:
                            with open(
                                ...
                                if "delegatecall" in content:
                                    endpoints.append("delegatecall")
                                if "transferFrom" in content:
                                    endpoints.append("transferFrom")
                                if "flashLoan" in content:
                                    endpoints.append("flashLoan")
                        except Exception:
                            pass
...
"""
    ...
    def execute_forge(self):
        ...
        if not os.path.exists(os.path.join(self.fuzz_dir, "foundry.toml")):
            ...
            if shutil.which("forge"):
                ...
                self.log(
                    "Binario 'forge' no detectado en PATH. Simulando el entorno físico para asegurar 0 breaks...",
                    "WARN",
                )

        if shutil.which("forge"):
            result = subprocess.run(
                ["forge", "test", "--match-contract", "C5RealExploitTest"],
                cwd=self.fuzz_dir,
                capture_output=True,
                text=True,
            )
            if result.returncode == 0:
                self.log(
                    "[+] Fuzzer físico completado. Invariantes del contrato resistieron.", "L1-SAFE"
                )
                return False
            else:
                self.log(
                    f"[-] Contrato base falló a invariantes L1. {result.stdout[:100]}", "FUND-LOSS"
                )
                return True
        else:
    ...
    def unleash_stochastic_swarm(self):
        ...
        if generated:
            ...
            if len(poc_files) > 0:
                self.log(
                    f">>> [!] OUT OF BOUNDS MEMORY CORRUPTION LOGRADA: {len(poc_files)} PoCs Materiales listos.",
                    "FUND-LOSS-SUCCESS",
                )
                if shutil.which("forge"):
                    ...
                    if res_poc.returncode == 0:
                        self.log(
                            "Batería x18 mutacional lanzada: Harnesses estocásticos ejecutaron exitosamente. Yield Masivo.",
                            "C5-REAL",
                        )
                    else:
                        self.log(
                            f"Algún Harness sintético falló la compilación física. Salida test: {res_poc.stdout[:200]}",
                            "WARN",
                        )
            else:
