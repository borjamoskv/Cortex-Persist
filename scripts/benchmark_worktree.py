...
from cortex.extensions.swarm.worktree_isolation import isolated_worktree, _git
...
async def benchmark():
    ...
    for i in range(3):
        ...
        async with isolated_worktree(f"{branch}-{i}") as wt_path:
            ...
            print(f"  Iteration {i}: Created in {(t1-t0)*1000:.1f}ms | User: {name} | FileMode: {filemode}")
