...
def autopoietic_heal_file(filepath: str) -> bool:
    ...
    logger.info(f"[LEA-Ω] Evaluación de Mutación: Entropía Original={penalty} -> Nueva={new_penalty}")
    
    if new_penalty < penalty:
        logger.info("[LEA-Ω] ✅ MUTACIÓN EXITOSA (Exergía incrementada). Aplicando sobreescritura C5-REAL.")
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(mutated_code + "\n")
        return True
    else:
        logger.warning("[LEA-Ω] ❌ Mutación rechazada (La entropía no mejoró). El código original sobrevive.")
        return False
...
def scan_and_heal(target_dir: str):
    ...
    for root, dirs, files in os.walk(target_dir):
        dirs[:] = [d for d in dirs if d not in (".venv", ".git", "__pycache__", "node_modules", "scripts")]
        for file in files:
            if file.endswith(".py"):
