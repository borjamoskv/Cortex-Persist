...
def analyze_redundancy(crystals: list[ScannedCrystal]) -> list[tuple[ScannedCrystal, ScannedCrystal, float]]:
    ...
    for project, proj_crystals in crystals_by_project.items():
        n = len(proj_crystals)
        if n < 2:
            continue
...
def main():
    ...
    parser.add_argument("--db-path", type=str, default="/Users/borjafernandezangulo/.cortex/cortex.db",
    ...
    print("\n" + "="*80)
    print(f" NIGHTSHIFT SEMANTIC MERGER (Ω₂) DRY-RUN ")
    print("="*80)
    ...
    if proposed_merges:
        ...
        for idx, (c1, c2, sim) in enumerate(proposed_merges, 1):
            ...
            prefix1 = c1.content[:80].replace('\n', ' ') + "..." if len(c1.content) > 80 else c1.content.replace('\n', ' ')
            print(f"        \"{prefix1}\"")
            print(f"    Crystal 2 (ID: {c2.fact_id}):")
            prefix2 = c2.content[:80].replace('\n', ' ') + "..." if len(c2.content) > 80 else c2.content.replace('\n', ' ')
            print(f"        \"{prefix2}\"")
            print("-" * 80)
