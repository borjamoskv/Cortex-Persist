...
def get_apollo_config() -> ApolloConfig:
    ...
    if not api_key:
        logger.error("MANDATE [APOLLO_API_GATE]: Abort immediately. API key missing. Zero fallback to C4-SIM.")
        sys.exit(1)
...
def extract_leads(config: ApolloConfig) -> list[dict]:
    ...
    if response.status_code != 200:
        logger.error(f"Extraction failed with status {response.status_code}. Payload: {response.text}")
        sys.exit(1)
