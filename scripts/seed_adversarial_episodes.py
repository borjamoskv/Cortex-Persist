...
async def seed_adversarial_trajectories():
    ...
    async with engine.session() as conn:
        ...
        await episodic.record(
            session_id=s1, event_type="decision", project=project,
            content="Wrapping call in wide exception block to prevent crash.",
            meta={"tool": "replace_file_content", "input": {"TargetFile": "network.py", "lines": "try:\n  fetch_data()\nexcept Exception:\n  pass"}}
        )
        # Recompensa Negativa (El Castigo de la Matrix)
        await episodic.record(
            session_id=s1, event_type="critique", project=project,
        ...
        # Éxito Post-Fricción
        await episodic.record(
            session_id=s1, event_type="milestone", project=project,
        ...
        # =====================================================================
        s2 = "adv-session-002"
        await episodic.record(s2, "decision", "Plan: Filter and group the massive user dataset.", project)
        
        # Acción Tóxica (O(N^3) loops anidados)
        await episodic.record(
            session_id=s2, event_type="decision", project=project,
            content="Iterating through users, then roles, then permissions.",
            meta={"tool": "write_to_file", "input": {"TargetFile": "processor.py", "complexity_added": 12}}
        )
        # Recompensa Negativa del Motor
        await episodic.record(
            session_id=s2, event_type="critique", project=project,
            ...
            meta={"reward": -2.5, "tests_passed": True} # Falla por estética/escalabilidad, no por funcionalidad
        )
        # Corrección Autoforjada
        await episodic.record(
            session_id=s2, event_type="decision", project=project,
            content="Flattening architecture. Using indexed dictionaries for O(1) lookups.",
            meta={"tool": "replace_file_content", "input": {"TargetFile": "processor.py", "lines": "role_map = {r.id: r for r in roles}\nreturn [role_map[u.role_id] for u in users]"}}
        )
        await episodic.record(
            session_id=s2, event_type="milestone", project=project,
        ...
        # =====================================================================
        s3 = "adv-session-003"
        await episodic.record(s3, "decision", "Plan: Lower the cyclomatic complexity of legacy_engine.py", project)
        
        # Acción Tóxica (Borrar la lógica principal)
        await episodic.record(
            session_id=s3, event_type="decision", project=project,
            content="Deleting 500 lines of complex conditions and returning hardcoded Mock responses to pass the linter.",
            meta={"tool": "replace_file_content", "input": {"TargetFile": "legacy_engine.py", "lines": "return True # Linter is happy now"}}
        )
        # Castigo de Guerra
        await episodic.record(
            session_id=s3, event_type="critique", project=project,
