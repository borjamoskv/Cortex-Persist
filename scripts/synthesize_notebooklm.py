...
def run():
    ...
    for project in projects:
        ...
        proj_orphans = df_orphans[df_orphans['project'] == project]['name'].tolist() if not df_orphans.empty else []
        unverified_count = df_unverified[df_unverified['project'] == project][['count']].sum().iloc[0] if not df_unverified.empty and project in df_unverified['project'].values else 0
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            if unverified_count > 0 or proj_orphans:
                ...
                f.write(f'> NotebookLM: He detectado las siguientes lagunas en CORTEX para este proyecto.\n')
                if unverified_count > 0:
                    f.write(f'- Hay **{unverified_count}** hechos sin verificar que requieren validación lógica.\n')
                if proj_orphans:
                    f.write(f"- Las siguientes entidades carecen de conexiones relacionales: {', '.join(proj_orphans[:5])}.\n")
                f.write('\n')
