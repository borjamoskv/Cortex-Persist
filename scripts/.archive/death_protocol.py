...
Implements AST-based parsing to detect structural code rot (AI slop, dead code, T_ODOs)
...
def check_file_entropy(filepath):
    ...
    # Penalty 2: AI Slop (T_ODOs, F_IXMEs)
    todos = len(re.findall(r"(?i)\b([t]odo|[f]ixme)\b", content))
    if todos > 0:
        penalties += todos * 2
        issues.append(f"AI Slop detected (T_ODOs/F_IXMEs). Penalty: +{todos * 2}")
