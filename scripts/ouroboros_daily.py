...
async def main():
    ...
    parser.add_argument(
        "--apply", action="store_true", help="Actually execute mutation (dry_run=False)"
    )
    ...
    for file_path in python_files:
        ...
        try:
            ...
            omega = OuroborosOmega(
                target_path=str(file_path), project_root=str(PROJECT_ROOT), dry_run=True
            )
            ...
            errors.append({"file": str(rel_path), "error": f"{type(e).__name__}: {str(e)}"})
            diagnoses.append(
                {
                    "file": str(rel_path),
                    "entropy": 0.0,
    ...
    for d in top_5:
        ...
        try:
            omega = OuroborosOmega(
                target_path=str(target_file), project_root=str(PROJECT_ROOT), dry_run=dry_run
            )
    ...
    avg_entropy = round(
        sum(d["entropy"] for d in diagnoses if d["status"] == "OK") / max(1, files_ok), 2
    )
