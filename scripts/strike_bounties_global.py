"""
CORTEX SOVEREIGN STRIKE: VECTOR A (GLOBAL)
Component: Unified Bounty Extractor (Algora, Polar, Immunefi).
...
logger = logging.getLogger("ouroboros.vector_a_global")

EXERGY_THRESHOLD: Final[float] = float(os.getenv("CORTEX_BOUNTY_MIN_USD", "100.0"))
...
async def execute_strike() -> None:
    logger.info("[VECTOR A] Ignition. Mapping sovereign capital vectors globally (Algora, Polar, etc).")
    ...
    async with aiosqlite.connect(db_path) as db:
        ...
        logger.info("[SCAN] Unleashing unified global scanner...")
        # Will run UnifiedScanner to get Algora, Polar, Immunefi, etc.
        leads = await bounty_svc.scan_all(min_usd=EXERGY_THRESHOLD)
        ...
        for lead in ranked_leads:
            ...
            logger.info(f"[TARGET LOCKED] {lead.repo}#{lead.number}: {lead.title}")
            ...
            justification = (
                ...
                f"  - Rango: [{float(ev) * 0.8:.2f}, {float(ev) * 1.2:.2f}] (execution variance)\n"
                f"  - Confianza: C3 (Evaluation) / C5-Dynamic (if executed)\n"
            )

            metrics: dict[str, Any] = {
                "bounty_id": f"{lead.repo}#{lead.number}",
                "target_url": lead.url,
                ...
                "entropy_delta": float(cost),
                "exergy_estimate": float(ev),
