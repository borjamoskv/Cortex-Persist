...
PERSONAL_PROJECTS = [
    'borja.moskv.eth', 'borjamoskv', 'borjamoskv.com', 'el-pueblo-online',
    'filete-cumbia', 'garmin-dashboard', 'xokas-elevator', 'gordacorp',
    'millennium', 'noir-ui-kit', 'openclaw', 'reddit_overlord',
    'songlines-manifestation-immunity', 'sonic-supreme', 'sonic_sovereign',
    'SonicNotch', 'veo-lyria-studio', 'lyria-studio', 'NAROA', 'LIVENOTCH',
    'eqmac-re', 'comienzos-clone', 'conspiracy-calculator', 'FrontierApp',
    'impact-web', 'manteca-web', 'moskvbot', 'moskvbot-test', 'OLA3',
    'omni-translate', 'raft-immunity'
]

# Repatriated to COLD STORAGE (Junk/Tests)
COLD_PROJECTS = [
    'test', 'test-project', 'test-integration-real', 'test_lateral_inhibition',
    'testing', 'test-integration', 'test-vec', 'vec-test', 'default',
    'other-project', 'blue', 'temp', 'cortex-test', 'swarm-demo'
]
...
def ensure_db_copy(src, dest):
    ...
    if not os.path.exists(dest):
        ...
        tables_to_clear = ['facts', 'episodes', 'transactions', 'entities', 'entity_relations', 'entity_events', 'memory_events']
...
def move_project_data(project_name, src_conn, dest_path):
    ...
    try:
        # 1. Facts
        facts = src_conn.execute("SELECT * FROM facts WHERE project = ?", (project_name,)).fetchall()
        if facts:
            columns = [d[0] for d in src_conn.execute("SELECT * FROM facts LIMIT 1").description]
            placeholders = ",".join(["?"] * len(columns))
            dest_conn.executemany(f"INSERT OR IGNORE INTO facts ({','.join(columns)}) VALUES ({placeholders})", facts)
            src_conn.execute("DELETE FROM facts WHERE project = ?", (project_name,))
        
        # 2. Episodes
        episodes = src_conn.execute("SELECT * FROM episodes WHERE project = ?", (project_name,)).fetchall()
        if episodes:
            columns = [d[0] for d in src_conn.execute("SELECT * FROM episodes LIMIT 1").description]
            placeholders = ",".join(["?"] * len(columns))
            dest_conn.executemany(f"INSERT OR IGNORE INTO episodes ({','.join(columns)}) VALUES ({placeholders})", episodes)
            src_conn.execute("DELETE FROM episodes WHERE project = ?", (project_name,))
            
        # 3. Transactions (linked to these projects)
        txs = src_conn.execute("SELECT * FROM transactions WHERE project = ?", (project_name,)).fetchall()
        if txs:
            columns = [d[0] for d in src_conn.execute("SELECT * FROM transactions LIMIT 1").description]
            placeholders = ",".join(["?"] * len(columns))
            dest_conn.executemany(f"INSERT OR IGNORE INTO transactions ({','.join(columns)}) VALUES ({placeholders})", txs)
