...
def scan_and_revoke(user_addr, private_key=None):
    ...
    for symbol, token_addr in TOKENS.items():
        ...
        for threat in threats:
            allowance = contract.functions.allowance(user_addr, w3.to_checksum_address(threat)).call()
            if allowance > 0:
                print(f"[🚨] ¡VESPELIGRO! Allowance de {allowance} detectada en {symbol} hacia {threat}")
                if private_key:
                    print(f"[*] Ejecutando REVOKE (Set to 0)...")
                    # Lógica de transacción aquí
                else:
                    print(f"[!] Se requiere Private Key para ejecutar la revocación automática.")
            else:
