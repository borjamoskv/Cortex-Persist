...
logger = logging.getLogger("inject_mythos_reveng")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas Mythos RevEng...")
    ...
    md_path = "AUTODIDACT_MYTHOS_REVENG.md"
    with open(md_path, encoding="utf-8") as f:
        content = f.read()

    pattern = re.compile(r'-\s*\*\*(MYTHOSREV-\d{3})\*\*:\s*`?([^`\n]+?)`?\s*-\s*(.*)')
    matches = pattern.findall(content)

    agent_id = 100
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "MYTHOSREV_ROOT", 0.0)
    ...
    for i, match in enumerate(matches):
        ...
        logger.info(f"Ingested {p_id} with hash {event_hash}")
        ultramap.update_agent_position(agent_id, (i + 1) * 1.0, 0.0, 0.0, "MYTHOSREV_LEAF", 0.1)
