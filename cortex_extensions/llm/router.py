...
from cortex_extensions.llm._cascade import CascadeManager, classify_tier
from cortex_extensions.llm._models import (
    BaseProvider,
    CascadeEvent,
    CascadeTier,
    CortexPrompt,
...
from cortex_extensions.llm._router_hedging import execute_hedged, execute_swarm
from cortex_extensions.llm._router_policy import ordered_fallbacks
from cortex_extensions.llm._router_shannon import compress_working_memory
from cortex_extensions.llm._telemetry import CascadeTelemetry
...
class CortexLLMRouter:
    ...
    async def route(
        ...
        if not provider_hint and prompt.system_instruction:
            ...
            try:
                from cortex_extensions.swarm.kv_prefix_registry import get_kv_registry
    ...
    def select_model_for_intent(self, intent: str) -> str | None:
        ...
        try:
            from cortex_extensions.llm._presets import resolve_model
    ...
    def optimal_provider_order(
        ...
        *,
        ...
        try:
            from cortex_extensions.llm._presets import providers_for_intent
    ...
    def frontier_order(intent: str) -> list[tuple[str, str]]:
        ...
        try:
            from cortex_extensions.llm._presets import frontier_providers
