...
class VoteRequest(BaseModel):
    agent: str = Field("human", max_length=100, description="Agent identifier casting the vote")
...
class CortexEngine:
    ...
    def vote(self, fact_id: int, agent: str, value: int) -> float:
        ...
        Args:
            ...
            agent: The name/ID of the agent voting.
...
async def recall_project(
    ...
    if include_deprecated:
        facts = await run_in_threadpool(api_state.engine.history, project)
    else:
        facts = await run_in_threadpool(api_state.engine.recall, project)
...
async def deprecate_fact(
    ...
    success = await run_in_threadpool(api_state.engine.deprecate, fact_id, reason=f"api:{auth.key_name}")
    if not success:
...
async def cast_vote(
    ...
    """Cast a consensus vote (verify/dispute) on a fact with tenant isolation."""
    try:
        # Verify fact exists and belongs to tenant
        def _verify_tenant():
        ...
        project = await run_in_threadpool(_verify_tenant)
        if project is None:
        ...
        score = await run_in_threadpool(api_state.engine.vote, fact_id, agent_id, req.vote)
...
@router.get("/v1/facts/{fact_id}/votes")
async def get_votes(
    fact_id: int,
    auth: AuthResult = Depends(require_permission("read")),
) -> list[dict]:
    """Get all votes for a specific fact with tenant isolation."""
    try:
        def _fetch_votes_with_check():
            with api_state.engine._get_conn() as conn:
                # 1. Check ownership
                row = conn.execute("SELECT project FROM facts WHERE id = ?", (fact_id,)).fetchone()
                if not row:
                    return None, "Fact not found"
                if row[0] != auth.tenant_id:
                    return None, "Forbidden"
                
                # 2. Fetch votes
                cursor = conn.execute(
                    "SELECT agent, vote, timestamp FROM consensus_votes WHERE fact_id = ?",
                    (fact_id,)
                )
                return [
                    {"agent": row[0], "vote": row[1], "timestamp": row[2]}
                    for row in cursor.fetchall()
                ], None

        votes, error = await run_in_threadpool(_fetch_votes_with_check)
        if error == "Fact not found":
            raise HTTPException(status_code=404, detail=f"Fact #{fact_id} not found")
        if error == "Forbidden":
            raise HTTPException(status_code=403, detail="Forbidden: Fact belongs to another tenant")
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch votes")
...
def client():
    ...
    test_db = "test_consensus_flow.db"
    ...
    api_state.engine = CortexEngine(test_db)
    api_state.engine.init_db()
    api_state.auth_manager = AuthManager(test_db)
    
    # Timing tracker
    timing_conn = sqlite3.connect(test_db, check_same_thread=False)
    api_state.tracker = TimingTracker(timing_conn)
    
    # Import routers and register them if not already done by lifespan 
    # (FastAPI TestClient handles lifespan if used with 'with')
    with TestClient(app) as c:
        # Create a test key
        raw_key, api_key = api_state.auth_manager.create_key("test_agent", tenant_id="test_proj")
...
def test_consensus_flow(client):
    ...
    # 3. Vote UP (verified threshold is +5 net votes)
    # Cast 5 votes from different agents
    ...
    # Engine direct votes to reach threshold
    for i in range(4):
...
def test_recall_ordering(client):
    ...
    fid_old = client.post("/v1/facts", json={"project": "test_proj", "content": "Verified Old"}).json()["fact_id"]
    for i in range(10): 
        api_state.engine.vote(fid_old, f"voter_{i}", 1) # Score = 2.0
        
    # Store new fact (Score = 1.0)
    fid_new = client.post("/v1/facts", json={"project": "test_proj", "content": "Stated New"}).json()["fact_id"]
    
    # Store disputed fact
    fid_bad = client.post("/v1/facts", json={"project": "test_proj", "content": "Disputed"}).json()["fact_id"]
    for i in range(6): 
    ...
    # Verified Old should be first because consensus is very high.
    # New should be second.
    # Disputed should be last.
