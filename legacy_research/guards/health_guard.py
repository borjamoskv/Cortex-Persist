...
try:
    from legacy_research.extensions.health.health_mixin import HealthMixin  # type: ignore
    from legacy_research.extensions.health.models import Grade, HealthSLA, HealthSLAViolation  # type: ignore
except ImportError:
...
class HealthGuard(HealthMixin):
    ...
    async def check_write_safety(self, custom_sla: HealthSLA | None = None) -> bool:
        ...
        Args:
            custom_sla: Optional SLA to check against. Defaults to DEFAULT_SLA.
        ...
        try:
            score = await self.health_score(persist=False) # type: ignore
        except TypeError:
