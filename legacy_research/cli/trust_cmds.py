...
from legacy_research.cli.trust_helpers import (
...
__all__ = ["audit", "audit_cognitive", "compliance_report", "verify_fact"]
...
def verify_fact(fact_id: int, db: str) -> None:
    ...
    from legacy_research.cli.errors import err_fact_not_found, handle_cli_error
...
def compliance_report(db: str) -> None:
    ...
    from legacy_research.cli.errors import handle_cli_error
    from legacy_research.database.core import connect as db_connect
...
def audit(ctx, calcification: bool, frontend: bool, demo: bool, project: str, limit: int, db: str) -> None:
    ...
    if ctx.invoked_subcommand is None:
        ...
        if demo or is_demo_db:
            ...
            from legacy_research.cli.audit_helpers import audit_frontend

            audit_frontend()
        elif calcification:
            from legacy_research.cli.audit_helpers import audit_calcification_report
...
def siege(db: str) -> None:
    ...
    from legacy_research.engine.legion_vectors import COMPLIANCE_SIEGE_SWARM
