...
def digest_cmd(output: str) -> None:
    ...
    async def _digest() -> None:
        ...
        limit_msg = "✅ Safe" if word_count < 500000 else "⚠️ OVER LIMIT"
        console.print(
            Panel(
...
def fragment_cmd(output_dir: str) -> None:
    ...
    async def _fragment() -> None:
        ...
        for domain in sorted(domain_facts.keys()):
            ...
            word_count = len(content.split())
            status = "[green]✅[/green]" if word_count < 500000 else "[red]⚠️ OVER[/red]"
...
def sync_cmd(drive_path: str | None, mode: str) -> None:
    ...
    console.print(
        Panel(
            f"[green]✅ Sincronizados {len(synced_files)} archivos a {provider_name}[/green]\n  Destino: {target}\n  {('Limpiados ' + str(cleaned) + ' archivos antiguos' if cleaned else '')}\n\n  📋 NotebookLM debería detectarlos automáticamente\n     si Drive está conectado como fuente.",
...
def status_cmd() -> None:
    ...
    if cloud:
        ...
        table.add_row("Cloud Sync", "Not detected", "-", "-", "[yellow]NO SYNC[/yellow]")
    console.print(table)
...
def ingest_cmd(drive_path: str | None) -> None:
    ...
    if manifest_path.exists():
        try:
            ...
            logger.warning("[NOTEBOOKLM] Manifest corrupted at %s, starting fresh.", manifest_path)
    engine = get_engine()

    async def _ingest() -> None:
        extracted_facts = []
        newly_processed = []
        system_prompt = 'You are a CORTEX integration parser operating under Axiom Ω₁ (Multi-Scale Causality). Extract discrete sovereign facts (decisions, ghosts, bridges, and knowledge) from the provided NotebookLM automated summary/notes.\nCRITICAL: If the text references any previous insights containing a \'Shadow Key\', which looks like \'[∆_CTX:xxxxxxxx]\' or \'∆_CTX:xxxxxxxx\' (or implies a connection to one), you MUST extract it.\nRespond ONLY with a raw JSON list of objects. No markdown formatting, no explanations.\nDo NOT include backticks (```json). Just the raw list in valid JSON.\nFormat:\n[\n  {\n    "fact_type": "decision|ghost|bridge|knowledge",\n    "project": "The associated project name",\n    "content": "The actual discovery or fact extracted",\n    "confidence": "C3",\n    "shadow_keys": ["∆_CTX:A1B2C3D4"] // Include if found, else empty list\\n  }\n]\nIf no facts are present, output an empty list: []'
        ...
        if extracted_facts:
            ...
            try:
                ...
                console.print(
                    Panel(
                        f"[green]✅ Ouroboros Loop Completado[/green]\nHechos asimilados: {len(ids)}\nArchivos procesados: {len(newly_processed)}",
