...
"""Cortex Router v4 - Nash-Equilibrium Retrieval Router.

Implements a game-theoretic mixture-of-experts routing system where
Flash (execution optimizer) and Pro (retrieval stability optimizer)
...
class NashCausalRouter(CausalPolicyGradientRouter):
    ...
    def __init__(
        self, alpha: float = 0.5, beta: float = 1.0, gamma: float = 1.5, delta: float = 0.8
    ):
        super().__init__()
    ...
    def compute_utilities(self, state: SignalVector) -> RoutingUtilities:
        ...
        misrouting_risk = max(
            0.0, 1.0 - state.kl_instability
        )  # Penalty if routed to Pro for trivial tasks
