...
class OuroborosLoop:
    ...
    def validate_in_sandbox(self) -> bool:
        ...
        sys.stdout.write(f"[OUROBOROS] Running hyper-vector sandbox validation...\\n")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                sys.stdout.write(f"[OUROBOROS] Validation failed (Entropy > 0).\\n{result.stderr}\\n")
                return False
        except FileNotFoundError:
             # pytest not found, assume failure or skip
             sys.stdout.write("[OUROBOROS] pytest not found. Sandbox validation bypassed (DANGEROUS).\\n")
             return True

    def integrate(self):
        """Writes the mutated AST back to disk."""
        sys.stdout.write(f"[OUROBOROS] Integrating mutation to disk...\\n")
        self.parser.save()
