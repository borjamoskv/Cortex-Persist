...
| **Simulation Plane** | Architecture | A sandbox mirroring the physical system state used to evaluate proposed cognitive policies prior to execution. |
| **ChallengeMessage** | Protocol | A typed contract for adversarial structured thinking, replacing absolute vetoes to avoid SPOF. |
...
interface BeliefObject {
  ...
  state: BeliefState;
  factuality: Factuality; // Epistemic origin class
  counterfactual_context?: string; // Optional namespace for alternate realities (e.g. "if_rollback")
...
### Epistemic Challenge (MUST)

An absolute epistemic veto is FORBIDDEN as it introduces a Single Point of Failure (SPOF).
Instead, conflict resolution MUST operate via a typed `ChallengeMessage` contract:

```typescript
interface ChallengeMessage {
    target_proposal: string; // UUID of challenged BeliefObject
    challenge_reason: string;
    confidence: number;
    evidence: string[]; // UUIDs of supporting evidence BeliefObjects
}
```
...
## 16. Simulation Plane (Mental Sandbox)

> **Classification: MUST (Normative)**

The system MUST NOT execute probabilistic LLM generations directly against the physical system state without prior simulation. `cortex.simulation` is NOT a generic mock layer; it is a full virtualized execution namespace.

### Requirements

- **Simulated Resources:** The sandbox MUST provide `SimulatedFilesystem`, `SimulatedMemory`, `SimulatedProcessTable`, and `SimulatedNetwork` bounded to the agent context.
...
The architecture permits runtime topology mutations and Just-In-Time (JIT) skill generation, provided they remain mathematically verifiable.

### JIT Skill Lifecycle

Any dynamic capability generated by the orchestrator (e.g., via `DEMIURGE-OMEGA`) MUST pass through a strict lifecycle before execution:
`Generation -> AST Validation -> Simulation -> Approval -> Activation`. Raw execution of unvalidated LLM output is structurally forbidden.

### Topology Governance

Orchestrators MUST NOT mutate the swarm topology (e.g., creating/destroying agent sub-networks) purely via internal memory variables. Topology mutations MUST be treated as sovereign state transitions and committed to the ledger:
- `TopologyChangeProposal`: Submits the intended mutation.
- `TopologyChangeApproval`: Quorum or L3 Auditor validation.
- `TopologyChangeCommit`: Execution and hash chain update.

## 18. Epistemic Governance
...
1. **Observed Reality:** Cryptographically signed telemetry from deterministic tools. Assigned maximum epistemic weight.
