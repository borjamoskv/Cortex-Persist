...
def create_document():
    ...
    doc.add_paragraph('\n\n\n\n')
    author = doc.add_paragraph('Análisis Técnico Exhaustivo\nAutor: Borja Fernández Angulo\nCORTEX System v4.0')
    ...
    row_data = [
        ('Transparencia y Trazabilidad', 'Log inmutable criptográfico de decisiones y acciones de los agentes (Chain of Thought Ledger).'),
        ('Supervisión Humana', 'Protocolos "Human-in-the-loop" obligatorios en operaciones críticas (C-Level Sandbox).'),
        ('Gestión de Riesgos', 'Evaluación estocástica y sandboxing estricto de ejecución de código (AST validation).')
    ]
    ...
    doc.add_paragraph("El núcleo de CORTEX es su motor de confianza, segmentado en cinco bi-capas estructurales para garantizar aislamiento absoluto y comunicación segura (Zero-Trust).")
    ...
    doc.add_paragraph("El sistema de memoria simula la cognición mamífera avanzada, permitiendo a los agentes razonar temporalmente sin colapso de contexto.")
    ...
    mem_levels = [
        ...
        ('L2: Episodic Memory', 'SQLiteDB (Transaccional)', 'Sesión / Proyecto (Indices semánticos).'),
    ...
    doc.add_paragraph("La integridad de los datos en CORTEX está garantizada a través de primitivas criptográficas robustas y un entorno de validación abstracto (AST). Cada modificación de estado genera un hash que se entrelaza en un Árbol de Merkle.")
    
    # Sección 6
    doc.add_heading(section_titles[5], level=1)
    doc.add_paragraph("La comunicación en enjambre requiere tolerancia a fallos. CORTEX implementa una variante de Tolerancia a Faltas Bizantinas Ponderada (WBFT).")
    ...
    consensus = [
        ('Leader Election', 'Rotación determinista basada en reputación histórica (proof-of-accuracy).'),
    ...
    doc.add_paragraph("Para mantener la robustez sin caer en el gigantismo de software, CORTEX restringe su código base enfocándose en la densidad y el determinismo.")
