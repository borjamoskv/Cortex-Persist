...
logger = logging.getLogger("inject_computronio")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas COMPUTRONIO...")
    ...
    md_path = "AUTODIDACT_COMPUTRONIO.md"
    with open(md_path, encoding="utf-8") as f:
        content = f.read()

    pattern = re.compile(r'-\s*\*\*([A-Z]+-\d{3})\*\*:\s*`?([^`\n]+?)`?\s*-\s*(.*)')
    matches = pattern.findall(content)

    agent_id = 99
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "COMPUTRONIO_ROOT", 0.0)
    ...
    for i, match in enumerate(matches):
        ...
        ultramap.update_agent_position(agent_id, (i + 1) * 1.0, 0.0, 0.0, "COMPUTRONIO_LEAF", 0.1)
