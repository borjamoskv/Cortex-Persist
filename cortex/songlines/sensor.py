...
class TopographicSensor:
    ...
    def _get_attr_payload(self, file_path: Path, attr: str) -> bytes | None:
        ...
        # 2. Try xattr CLI -p
        try:
            return subprocess.check_output(['xattr', '-p', attr, str(file_path)], stderr=subprocess.DEVNULL)
        except (subprocess.SubprocessError, FileNotFoundError):
            return None

    def _parse_ghost_payload(self, file_path: Path, attr: str, payload_bytes: bytes) -> dict[str, Any] | None:
