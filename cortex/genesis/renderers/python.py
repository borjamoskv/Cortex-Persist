...
from cortex.genesis.renderers.utils import _ensure_self_param
...
def _render_module(system_name: str, comp: ComponentSpec) -> str:
    ...
    parts.extend([
        "",
        f"class {class_name}:",
        f'    """Manager for {comp.name} operations."""',
        ""
    ])
...
def _render_test(system_name: str, comp: ComponentSpec) -> str:
    ...
    parts: list[str] = []
    parts.append(f'"""Tests for {system_name}.{comp.name}."""\n\nfrom __future__ import annotations\n')
    if comp.imports:
        parts.extend(sorted(comp.imports))
        parts.append("")
...
def _render_protocol(system_name: str, comp: ComponentSpec) -> str:
    ...
    if comp.interfaces:
        for iface in comp.interfaces:
            ...
            parts.extend([
                f"    def {method}(self, **kwargs: Any) -> Any:",
                "        ...",
                ""
            ])
    else:
