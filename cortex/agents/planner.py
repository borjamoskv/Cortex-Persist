...
class PlanStep:
    ...
    exergy_estimate: Decimal = Decimal("0.5")  # Expected work yield [0.0, 1.0]
    entropy_cost: Decimal = Decimal("0.1")  # Expected entropy paid [0.0, 1.0]
    ...
    def net_exergy(self) -> float:
        ...
        return max(0.0, float(self.exergy_estimate) - float(self.entropy_cost))
...
class ExecutionPlan:
    ...
    total_exergy_produced: Decimal = Decimal("0.0")
    total_entropy_paid: Decimal = Decimal("0.0")

    @property
    def net_exergy(self) -> float:
        return float(self.total_exergy_produced) - float(self.total_entropy_paid)
