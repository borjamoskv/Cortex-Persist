...
class BabestuTCell:
    ...
    FORBIDDEN_CALLS = {"eval", "exec", "compile", "__import__", "subprocess", "os.system", "os.popen"}
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                if isinstance(node.func, ast.Name) and node.func.id in cls.FORBIDDEN_CALLS:
                    return False, f"Llamada a ejecución dinámica o sistema prohibida: {node.func.id}()"
    ...
    def scan_payload(cls, raw_text: str) -> Dict[str, Any]:
        ...
        if cls.B64_HEURISTIC.search(raw_text):
            return cls._veredicto("CONTAMINADO", 90, "Base64_Obfuscation_Suspected", "Cadena Base64 inusualmente larga detectada.")
            
        if cls.HEX_HEURISTIC.search(raw_text):
            return cls._veredicto("CONTAMINADO", 95, "Hex_Obfuscation_Suspected", "Secuencia HexByte ofuscada detectada.")
            
        python_blocks = re.findall(r'```python\n(.*?)\n```', raw_text, re.DOTALL | re.IGNORECASE)
        for idx, block in enumerate(python_blocks):
            is_safe, reason = cls.analyze_python_ast(block)
            if not is_safe:
                return cls._veredicto("CONTAMINADO", 100, "AST_Static_Lethal_Vector", f"Bloque {idx}: {reason}")
