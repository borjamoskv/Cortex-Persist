...
from cortex.jules.models import TesterOutput
from cortex.jules.tools import AgentToolkit

__all__ = ["TesterAgent"]

logger = logging.getLogger("cortex.jules.tester")
...
class TesterAgent:
    ...
    def _infer_pass(output: str, cmd: str) -> bool:
        ...
        if "no tests ran" in lower or "no test" in lower:
            return True  # no tests = no failure from Jules's perspective
