"""MOSKV-Jules — Main JulesAgent orchestrator.
...
from cortex.jules.critic import CriticAgent
from cortex.jules.executor import ExecutorAgent
from cortex.jules.models import AgentTask, TaskStatus
from cortex.jules.planner import PlannerAgent
from cortex.jules.queue import TaskQueue
from cortex.jules.tester import TesterAgent
from cortex.jules.tools import AgentToolkit

__all__ = ["JulesAgent"]

logger = logging.getLogger("cortex.jules.runner")

_MAX_EXECUTOR_RETRIES = 1  # Critic can send back once for fixes


class JulesAgent:
    """Sovereign autonomous coding agent — Jules-like, local-first.
    ...
    async def run_task(self, task: AgentTask, queue: TaskQueue) -> AgentTask:
        ...
        logger.info("🤖 Jules starting task [%s] — %s", task.id, task.title)

        branch = f"jules/{task.id}"
        ...
        if diff.strip() and not diff.startswith("[ERROR]"):
            toolkit.git_commit(f"jules({task.id}): {task.title[:60]}")
        ...
        # macOS notification
        self._notify(f"Jules ✅ [{task.id}]", task.title)
    ...
    def _fail(self, task: AgentTask, queue: TaskQueue, error: str) -> AgentTask:
        ...
        self._notify(f"Jules ❌ [{task.id}]", f"Failed: {error[:80]}")
    ...
    def _persist_to_cortex(task: AgentTask, result: str) -> None:
        ...
        try:
            ...
            msg = f"Jules completed task [{task.id}]: {task.title}. Branch: {task.branch}. Result: {result[:200]}"
            subprocess.run(
                [
    ...
    def _persist_error_to_cortex(task: AgentTask, error: str) -> None:
        ...
        try:
            ...
            msg = f"Jules failed task [{task.id}]: {task.title}. Error: {error[:300]}"
            subprocess.run(
                [
                    "python", "-m", "cortex.cli", "store",
                    "--type", "error",
