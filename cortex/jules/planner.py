...
from cortex.jules.models import PlanOutput
from cortex.jules.tools import AgentToolkit

__all__ = ["PlannerAgent"]

logger = logging.getLogger("cortex.jules.planner")

_SYSTEM = """You are a Sovereign Architect AI (Planner). Your job is to analyze
a codebase and produce a precise, actionable implementation plan for a given task.
...
class PlannerAgent:
    ...
    def _tree(root: Path, depth: int, prefix: str = "", current: int = 0) -> str:
        ...
        try:
            ...
            return ""
        ignore = {".git", "__pycache__", ".venv", "node_modules", ".build",
                  "dist", ".mypy_cache", ".pytest_cache"}
        for e in entries:
            if e.name in ignore or e.name.startswith("."):
                continue
            lines.append(f"{prefix}{'└── ' if e == entries[-1] else '├── '}{e.name}{'/' if e.is_dir() else ''}")
            if e.is_dir():
