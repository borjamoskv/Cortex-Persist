...
class SourceMetadata(BaseModel):
    ...
    author: str = Field(
        default="", description="Specific entity (e.g., username, agent_id)."
    )
    document_ref: str = Field(
        default="", description="Reference to external source if any."
    )
...
class MemoryAccessStats(BaseModel):
    ...
    retrieval_failure_count: int = Field(
        default=0, description="Consecutive or total failures."
    )
    total_access_count: int = Field(
        default=0, description="Total times this memory was recalled."
    )
...
class CortexFactModel(BaseModel):
    ...
    source_metadata: SourceMetadata = Field(
        ...
        description="Provenance of the fact for Source Monitoring."
    )
    access_stats: MemoryAccessStats = Field(
        default_factory=MemoryAccessStats,
        description="Metamemory usage stats."
    )
    ...
    def update_on_read(self, latency_ms: float = 0.0) -> CortexFactModel:
        ...
        new_stats = stats.model_copy(update={
