...
class TemporalCache(BaseModel):
    ...
    access_frequency: int = Field(
        default=1, description="Number of times accessed in current cycle"
    )
...
class ConsolidatedMemoryGraph(BaseModel):
    ...
    synaptic_connections: list[str] = Field(
        default_factory=list, description="IDs of related nodes"
    )
    dopamine_weight: float = Field(default=1.0, description="Connection strength based on TD-error")
    reputation_proof: float = Field(
        description="Aggregated reputation score of agents that validated this node"
    )
