...
def _git_tag() -> str:
    """Return the latest git tag or 'v1.0.0'."""
...
def _generate_live_prompt(project_root: Path) -> str:
    ...
    from cortex_extensions.agents.system_prompt import SYSTEM_PROMPT
...
def prompt_show(variant: str) -> None:
    ...
    from cortex_extensions.agents.system_prompt import (
        SYSTEM_PROMPT,
...
def prompt_generate(variant: str, out: str | None) -> None:
    ...
    if variant == "full":
        ...
        from cortex_extensions.agents.system_prompt import SYSTEM_PROMPT_MEDIUM, SYSTEM_PROMPT_SHORT
