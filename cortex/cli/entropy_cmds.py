...
def entropy_install_hook():
    ...
    if not git_dir.exists():
        console.print("[bold red]❌ No se encontró un repositorio Git en este directorio.[/]")
        return
    ...
    if not source_script.exists():
        console.print(f"[bold red]❌ No se encontró el script zero_debt.py en {source_script}[/]")
        return
...
def entropy_report():
    ...
    if not status_dict:
        console.print(
            "[dim]No se encontró estado del daemon. Asegúrate de que MoskvDaemon esté corriendo.[/]"
        )
        return
