...
async def submit_vote(req_id: str) -> None:
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT state_payload FROM quorum_requests WHERE id = ? AND status = 'PENDING'", (req_id,))
        row = cursor.fetchone()
        if not row:
    ...
    success = await auth_gw.submit_vote(req_id, signature_b64=signature, public_key_b64=pub_key, semantic_truth=True)
    if success:
...
async def list_requests() -> None:
    ...
    try:
        conn = engine.pool.get_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, status, hypothesis, signatures_json FROM quorum_requests WHERE status = 'PENDING'")
        ...
        for row in rows:
            ...
            console.print(f"[bold cyan]{row[0]}[/bold cyan] | [yellow]{row[1]}[/yellow] | Votes: {len(sigs)} | {row[2]}")
    except Exception as e:
