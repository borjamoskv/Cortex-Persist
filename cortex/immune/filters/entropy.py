...
class EntropyFilter(ImmuneFilter):
    ...
    async def evaluate(self, signal: Any, context: dict[str, Any]) -> FilterResult:
        ...
        if entropy_delta > 10.0:
            ...
            justification = f"Action adds too much entropy ({entropy_delta:.2f}). Rejected by Axiom Ω₂."
        elif entropy_delta > 0.0:
            verdict = Verdict.HOLD
            justification = f"Net entropy positive ({entropy_delta:.2f}). Justify why complexity is necessary."
