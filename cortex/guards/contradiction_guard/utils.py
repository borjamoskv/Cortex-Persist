"""
Utility functions for text processing, tokenization, and classification in the Contradiction Guard.
"""
...
DecryptFn: TypeAlias = Callable[[str], str]
...
@functools.lru_cache(maxsize=1024)
def _extract_versions(content: str) -> tuple[str, ...]:
    """Extract version numbers from content."""
    return tuple(_VERSION_PATTERN.findall(content))
...
def _decrypt_content(content: str, decrypt_fn: DecryptFn | None) -> str | None:
...
def _classify_conflict(
    ...
    new_tokens: frozenset[str] | set[str],
    existing_tokens: frozenset[str] | set[str],
    base_score: float,
) -> tuple[str, float]:
