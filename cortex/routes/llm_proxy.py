...
class DeterministicLabyrinth:
    ...
    def annihilate_entropy(raw_text: str) -> str:
        ...
        if not has_reality_level:
            exergy_output.append("> [LABYRINTH_WARNING]: Implicit C4-SIM state. Reality level undeclared.\n")
...
async def proxy_chat_completions(
    ...
    if not UPSTREAM_API_KEY:
        raise HTTPException(status_code=500, detail="EXERGY_UPSTREAM_KEY not configured in environment.")
    ...
    if is_stream:
        return StreamingResponse(
            _stream_labyrinth_proxy(request, payload),
