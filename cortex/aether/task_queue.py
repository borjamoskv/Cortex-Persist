...
logger = logging.getLogger("cortex.aether.task_queue")
...
class TaskQueue:
    ...
    def pop_next(self) -> AgentTask | None:
        ...
        Uses SQLite 3.35+ UPDATE ... RETURNING for true process-level O(1) atomicity,
        bypassing the need for GIL/thread locks. This ensures zero race conditions
        even if multiple MOSKV-1 agents pull from the queue simultaneously.
        """
        now = datetime.now(timezone.utc).isoformat()

        with self._conn() as conn:
            # Atomic pop using SQLite native RETURNING clause.
            # Subquery finds the oldest pending task using the (status, created_at) index.
            row = conn.execute(
                """
                UPDATE agent_tasks
