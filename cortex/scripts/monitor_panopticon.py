...
# Cargar variables de entorno (ALCHEMY_API_KEY, TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID)
load_dotenv()
...
# ---------------------------------------------------------
# TARGETS (Wallets del atacante Foizur / arbithumarb.eth cluster)
# ---------------------------------------------------------
TARGET_WALLETS = [
    ...
    "0x7df263b72c722f46ba54589d9e4672642",  # arbithumarb.eth activo
    "0x06060c5E3A090A1aFF282BBeC1eB7Db7bdab7a60",  # Old Primary
...
# ---------------------------------------------------------
# DESTINOS CEX CONOCIDOS (Firmas de Hot Wallets de Exchanges)
# Para trackear cuándo intenta hacer cash out a Fiat.
# ---------------------------------------------------------
CEX_HOT_WALLETS = {
    # Su punto de salida confirmado en ARKHAM:
    "0x1Ad7C70B43387F7E8C4291640E1726EA6CCB0366".lower(): "Binance (Hacker KYC Exit)",
    # KuCoin (El origen de sus fondos KYC originales)
    "0x2b5634c42055806a59e9107ed44d43c426e58258": "KuCoin",
    "0x6e9f187aaeec0098dfc382ce2fdc9643d93fd995": "KuCoin 2",
    "0xf3fdd3ff129dca44bc09916681ac2eedf50ea7ff": "KuCoin 3",
    "0x63351336421d01af3344b58e658adcb73d6ebef4": "KuCoin 4 (Funder original)",
}

# ---------------------------------------------------------
# INFRAESTRUCTURA ALCHEMY (Base, Optimism, Arbitrum, Ethereum)
# ---------------------------------------------------------
CHAINS = {
...
processed_txs = deque(maxlen=1000)


def alert_telegram(message: str):
    """Envía alerta vía Telegram al sistema de mando de CORTEX"""
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        print(f"⚠️  [ALERTA PENDIENTE - NO TELEGRAM CONFIG]:\n{message}")
        return
...
def check_address_activity(chain_name: str, rpc_url: str, address: str):
    """Consulta las transacciones recientes de la dirección en la cadena"""
    payload = {
        "jsonrpc": "2.0",
        "id": 1,
        ...
        "params": [
            {
                "fromBlock": "0x0",  # En producción cambiar a los últimos 100 bloques
                "toBlock": "latest",
                "fromAddress": address,
                "category": ["external", "erc20", "erc721", "erc1155"],
                "withMetadata": True,
                "excludeZeroValue": True,
                "maxCount": "0x5",  # Solo las últimas 5 para no saturar
            }
        ],
    }
    ...
    try:
        ...
        if "result" in data and "transfers" in data["result"]:
            for tx in data["result"]["transfers"]:
                ...
                asset = tx.get("asset", "UNK")

                # Check 1: Transferencias hacia CEX conocidos
                if to_addr and to_addr.lower() in CEX_HOT_WALLETS:
                    cex_name = CEX_HOT_WALLETS[to_addr.lower()]
                    msg = (
                        f"🚨 **PANÓPTICO ALERT: INTENTO DE CASH-OUT** 🚨\n\n"
                        f"🎯 **Target:** `{address}`\n"
                        f"🏢 **Destino:** {cex_name} (`{to_addr}`)\n"
                        f"💰 **Monto:** {value} {asset}\n"
                        f"⛓️ **Cadena:** {chain_name}\n"
                        f"🔗 **TxHash:** `{tx_hash}`\n\n"
                        f"⚠️ **ACCIÓN REQUERIDA:** Enviar dossier KuCoin Compliance/Binance Legal inmediatamente."
                    )
                    alert_telegram(msg)
                    print(msg)

                # Check 2: Fondos en movimiento general
                else:
                    print(
                        f"[{chain_name}] Movimiento detectado: {address} -> {to_addr} ({value} {asset})"
                    )

    except Exception as e:
        print(f"Error consultando {rpc_url}: {e}")


def run_panopticon():
    print("👁️  INICIANDO EL PANÓPTICO (Sovereign Watcher v1.0)")
    print(
        f"📡 Monitoreando {len(TARGET_WALLETS)} wallets atacantes a través de {len(CHAINS)} cadenas EVM."
    )
    print("⏳ Escaneando cada 60 segundos...\n")

    while True:
        try:
            for chain_name, rpc_url in CHAINS.items():
                for wallet in TARGET_WALLETS:
                    check_address_activity(chain_name, rpc_url, wallet.lower())

            # Rate limit básico
            time.sleep(60)

        except KeyboardInterrupt:
            print("\n🛑 Apagando el Panóptico. Las wallets quedan sin vigilancia.")
            break
        except Exception as e:
