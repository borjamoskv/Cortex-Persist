...
class LedgerAuditMixin:
    async def audit_integrity_async(self, tenant_id: str | None = None) -> dict:
        ...
        async with self._get_conn_proxy() as conn:
            ...
            chain_violations, tx_count, store_txs_to_verify, purged_store_tx_ids = await self._verify_chain(conn, tenant_id)
            violations.extend(chain_violations)
            
            # 2. Verify facts
            facts_violations = await self._verify_facts(conn, tenant_id, store_txs_to_verify, purged_store_tx_ids)
    ...
    async def _verify_chain(self, conn, tenant_id: str | None) -> tuple[list[dict], int, dict[int, str], set[int]]:
        violations = []
        tx_count = 0
    ...
    async def _verify_facts(self, conn, tenant_id: str | None, store_txs_to_verify: dict[int, str], purged_store_tx_ids: set[int]) -> list[dict]:
        violations = []
        cursor_check = await conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='facts'")
        ...
        if tenant_id is None:
            fact_cursor = await conn.execute("SELECT tx_id, content, hash, COALESCE(tenant_id, 'default') FROM facts")
        else:
        ...
        for f_tx_id, info in list(active_facts_by_tx.items()):
            try:
                ...
                violations.append({"id": f_tx_id, "type": "FACT_DECRYPTION_FAILED", "error": str(e)})
        ...
        for store_tid, expected_hash in store_txs_to_verify.items():
            if store_tid in active_facts_by_tx:
                ...
                violations.append({"id": store_tid, "type": "FACT_MISSING", "expected_hash": expected_hash})
    ...
    async def _verify_merkle_roots(self, conn, tenant_id: str | None) -> list[dict]:
        violations = []
        if tenant_id is None:
            cursor = await conn.execute("SELECT COALESCE(tenant_id, 'default'), root_hash, tx_start_id, tx_end_id FROM merkle_roots")
        else:
            cursor = await conn.execute(
