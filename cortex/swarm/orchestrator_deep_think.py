...
from cortex.agents.factory import create_agent
...
logger = logging.getLogger("cortex.swarm.deep_think")
...
class DeepThinkOrchestrator:
    ...
    def __init__(self, engine: Any, minds_manager: InfiniteMindsManager):
        ...
        self.agents_ids = [
            "julen_guerrero_omega", "maldini_arch_omega", "zubizarreta_redteam_omega",
            "dunga_chaos_omega", "romario_data_omega", "baggio_sync_omega",
            "stoichkov_finance_omega", "bebeto_yield_omega",
            "zubi_bridge_omega", "valderrama_vision_omega",
        ]
    ...
    async def _gather_vector_perspectives(self, context: str, project: str, sem: asyncio.Semaphore) -> list[dict[str, Any]]:
        """Recopila razonamientos en crudo y sus firmas HDC."""
        tasks = [self._get_vector_reasoning(agent_id, context, project, sem) for agent_id in self.agents_ids]
        return list(await asyncio.gather(*tasks))

    async def _get_vector_reasoning(self, agent_id: str, context: str, project: str, sem: asyncio.Semaphore) -> dict[str, Any]:
        """Invoca el agente, extrae su texto y lo comprime a HDC."""
        bus = await self.engine.get_bus()
        agent = create_agent(agent_id, self.engine.memory.l3, bus)
    ...
    async def _maradona_synthesis(
        ...
        bus = await self.engine.get_bus()
        maradona = create_agent("maradona_10_omega", self.engine.memory.l3, bus)
