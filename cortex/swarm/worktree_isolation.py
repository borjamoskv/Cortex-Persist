...
@asynccontextmanager
async def isolated_worktree(branch_name: str, base_path: str | Path | None = None) -> AsyncGenerator[Path, None]:
    """
    ...
    logger.info("🌿 [WORKTREE ISOLATION] Bipartición del espacio-tiempo. Creando worktree en: %s (Branch: %s)", worktree_path, branch_name)
    ...
    try:
        ...
        if proc.returncode != 0:
            raise WorktreeIsolationError(f"No estamos en un repositorio Git válido. Imposible bifurcar: {stderr_chk.decode().strip()}")
    ...
    try:
        ...
        logger.info("🔥 [WORKTREE ISOLATION] Colapso de función de onda. Purgando worktree obsoleto: %s", worktree_path)
        try:
