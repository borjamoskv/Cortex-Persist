...
class SunoV5Adapter(AudioAdapter):
    ...
    async def generate(self, prompt_matrix: dict[str, Any]) -> str:
        logger.info(
            "Enviando matriz paramétrica a la API de Suno v5... BPM: %s",
            prompt_matrix.get("bpm")
        )
...
class UdioV4Adapter(AudioAdapter):
    ...
    async def generate(self, prompt_matrix: dict[str, Any]) -> str:
        logger.info(
            "Iniciando síntesis en Udio v4 API... Escala: %s",
            prompt_matrix.get("key")
        )
        # Inject sonic vectors for premium texture
        sonic_v = prompt_matrix.get("sonic_vectors", {})
        enhanced_prompt = prompt_matrix.get("prompt_injection", "")
    ...
    async def magic_edit(self, track_uri: str, segment: tuple[float, float], edit_prompt: str) -> str:
...
class Lyria3Adapter(AudioAdapter):
    ...
    async def generate(self, prompt_matrix: dict[str, Any]) -> str:
        ...
        payload = {
            "prompt": enhanced_prompt,
            "parameters": {
                "bpm": prompt_matrix.get("bpm"),
                "key": prompt_matrix.get("key")
            }
        }
        try:
