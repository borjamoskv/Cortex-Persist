...
MODELS = {
    "gemini_3_1_pro": Model(
        ...
        version="3.1",
        capabilities=ModelCapabilities(
            reasoning_depth=9, tool_use=False, stochasticity=0.2, latency_ms=1200
        ),
    ),
    "gemini_3_5_flash": Model(
        ...
        version="3.5",
        capabilities=ModelCapabilities(
            reasoning_depth=5, tool_use=True, stochasticity=0.7, latency_ms=300
        ),
    ),
}
...
class PipelineStage:
    ...
    func: (
        Literal["KLTransform", "CoxFit", "ZScoreNormalize", "ContinuousCalibrationTransform"] | None
    ) = None
    mode: Literal["append_only"] | None = None
...
ROUTES = [
    ...
    RouteRule(
        when=RouteCondition(
            domain=TaskDomain.ARCHITECTURE_REVIEW, risk=RiskLevel.EPISTEMIC_CRITICAL
        ),
        assign=RouteAssignment(
            planner=MODELS["gemini_3_1_pro"],
