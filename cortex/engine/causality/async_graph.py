...
from cortex.engine.causality.models import TaintStatusMixin


class AsyncCausalGraph(TaintStatusMixin):
    ...
    async def _metadata_column(self) -> str | None:
        ...
        return "metadata" if "metadata" in cols else "meta" if "meta" in cols else None
    ...
    async def _get_descendant_ids(self, fact_id: int, tenant_id: str) -> set[int]:
        ...
        WITH RECURSIVE descendants AS (
            SELECT ? AS id UNION SELECT ce.fact_id FROM causal_edges ce JOIN descendants d ON ce.parent_id = d.id WHERE ce.tenant_id = ? AND ce.edge_type != ?
        ) SELECT id FROM descendants""",
            (fact_id, tenant_id, EDGE_TAINTED_BY),
        ) as cursor:
            async for row in cursor:
                ids.add(int(row[0]))
        return ids
    ...
    async def _fetch_edges(self, node_ids: list[int], tenant_id: str) -> dict[int, list[int]]:
        ...
        for i in range(0, len(node_ids), 900):
            ...
            ph = ",".join(["?"] * len(chunk))
            sql = f"SELECT fact_id, parent_id FROM causal_edges WHERE fact_id IN ({ph}) AND edge_type != ? AND tenant_id = ?"
            async with self.conn.execute(sql, (*chunk, EDGE_TAINTED_BY, tenant_id)) as cur:
                async for cid, pid in cur:
                    if pid is not None:
                        edges.setdefault(int(cid), []).append(int(pid))
        return edges

    async def _fetch_nodes_data(
        self, node_ids: list[int], tenant_id: str, meta_col: str | None, has_tenant: bool
        ...
        for i in range(0, len(node_ids), 900):
            chunk = node_ids[i : i + 900]
            ph = ",".join(["?"] * len(chunk))
            sql = f"SELECT id, confidence{f', {meta_col}' if meta_col else ''} FROM facts WHERE id IN ({ph})"
            params = list(chunk)
            if has_tenant:
                sql += " AND tenant_id = ?"
            ...
            async with self.conn.execute(sql, params) as cursor:
                async for row in cursor:
                    raw = row[2] if meta_col and len(row) > 2 else "{}"
                    m, ij, ie = self._parse_metadata(raw, tenant_id, enc)
                    nodes_data[int(row[0])] = {
                        "confidence": row[1] or "C5",
                        "metadata": m,
                        "is_json": ij,
                        "is_encrypted": ie,
                        "raw_meta": raw,
    ...
    def _parse_metadata(
        ...
        if isinstance(raw_meta, str) and raw_meta.startswith(enc.PREFIX):
            try:
                return enc.decrypt_json(raw_meta, tenant_id=tenant_id) or {}, True, True
            except (ValueError, TypeError):
                return {}, False, False
        try:
            return json.loads(raw_meta), True, False
    ...
    def _process_taint_propagation(
        ...
        while queue:
            ...
            new_status = self._derive_node_status(
                curr_id, source_id, edges, nodes_data, node_states, TaintStatus
            )
    ...
    async def _apply_fact_updates(
        ...
        enc = get_default_encrypter()
        for chg in changes:
            fid, data, new_conf = chg["fact_id"], nodes_data[chg["fact_id"]], chg["new_confidence"]
            if meta_col:
                payload = (
                    enc.encrypt_json(data["metadata"], tenant_id=tenant_id)
                    if data["is_encrypted"]
                    else (
                        json.dumps(data["metadata"])
                        if data["is_json"]
                        else data.get("raw_meta", "")
                    )
                )
                row = (new_conf, payload, fid)
            else:
                row = (new_conf, fid)
            fact_updates.append((*row, tenant_id) if has_tenant else row)
        sql = (
            "UPDATE facts SET confidence = ?"
            + (f", {meta_col} = ?" if meta_col else "")
            + " WHERE id = ?"
            + (" AND tenant_id = ?" if has_tenant else "")
        )
        await self.conn.executemany(sql, fact_updates)

    async def _record_taint_edges(
        self, changes: list[dict[str, Any]], source_id: int, has_tenant: bool, tenant_id: str
    ) -> None:
        params = [
            tuple(
                [c["fact_id"], source_id, None, EDGE_TAINTED_BY, None]
                + ([tenant_id] if has_tenant else [])
            )
            for c in changes
            if c["fact_id"] != source_id
        ]
        if not params:
            return
        sql = (
