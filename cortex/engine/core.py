...
class AsyncCortexEngine(
    ...
    def __init__(
        ...
        # Composition layers (Lazy Init to avoid import loops)
        ...
        # Decoupled guard pipeline (Ω₃: minimal coupling — Lazy Init)
        self._guard_pipeline: Optional[GuardPipeline] = None
    ...
    @property
    def guard_pipeline(self) -> GuardPipeline:
        """Access the guard pipeline, initializing it on first use."""
    ...
    async def get_conn(self) -> Any:
        ...
        async with self._conn_lock:
            ...
            try:
                ...
                # Defensive check for loadable_path() to prevent hangs
    ...
    def _get_sync_conn(self) -> Any:
        ...
        if sqlite_vec:
            try:
                ...
                pass  # System Python or missing library
        return conn

    # ─── Synchronous Wrappers (SDK Parity) ────────────────────────

    def _run_sync(self, coro):
        """Execute a coroutine synchronously, thread-safe, with timeouts."""
        timeout_sec = 10.0  # Reduced to 10s for fast failure, prevents huge hangs
        ...
        def _worker():
            ...
            try:
                result = asyncio.run(asyncio.wait_for(coro, timeout=timeout_sec))
            except Exception as e:
                # Catch Timeouterror and CancelledError correctly
                exception = e
    ...
    async def get_fact(self, *args, **kwargs):
        ...
        fields = Fact.__dataclass_fields__
        return Fact(**{k: v for k, v in res.items() if k in fields})
