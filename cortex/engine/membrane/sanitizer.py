...
from .models import PureEngram, MembraneLog, MembraneLogLevel
...
class SovereignSanitizer:
    ...
    TRACEBACK_REGEX = re.compile(r"Traceback\s*\(most\s*recent\s*call\s*last\):[\s\S]*?(?=\n[A-Za-z]+Error:|\Z)", re.IGNORECASE)
    ...
    def digest(cls, raw_engram: Dict[str, Any]) -> Tuple[PureEngram, MembraneLog]:
        ...
        """
        start_time = time.time() if 'time' in globals() else 0 # handle missing import
