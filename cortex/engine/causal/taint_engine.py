...
def generate_secure_taint_token(agent_id: str, session_id: str, content: str, private_key_b64: str, nonce: str | None = None) -> str:
...
async def _query_agent_key_async(conn, agent_id: str) -> str | None:
    try:
        cursor = await conn.execute("SELECT public_key FROM agents WHERE id = ? AND is_active = 1", (agent_id,))
...
async def _check_and_register_nonce_async(conn, nonce: str) -> bool:
    ...
    if await cursor.fetchone():
        return False
    await conn.execute("INSERT INTO taint_nonces (nonce, timestamp) VALUES (?, ?)", (nonce, time.time()))
    await conn.commit()
    return True
...
async def verify_taint_token(conn, token: str | None, content: str) -> bool:
    ...
    try:
        ...
        logger.error("[TaintEngine] SAGA-1: Cryptographic signature verification failed: %s", sig_err)
        return False
...
async def enforce_taint_check(conn, token: str | None, content: str) -> None:
    ...
    if not is_valid:
        raise TaintValidationError("SAGA-1 Rejection: Valid cryptographically signed CORTEX-TAINT token is required.")
