...
class EntropyDemon:
    ...
    async def attack(self, code: str, context: dict[str, Any]) -> list[str]:
        ...
        if "except Exception:" in code or "except:" in code:
            findings.append("Fragility: Bare `except` detected. System cannot tolerate undetected entropy.")
...
class ChronosSniper:
    ...
    async def attack(self, code: str, context: dict[str, Any]) -> list[str]:
        ...
        if "async def" in code:
            ...
            findings.append("Blocking Logic: Synchronous `time.sleep` in logic that requires asynchrony.")

            # Check for shared state without locks in pseudo-code intent
            if "global " in code:
                findings.append("Race Condition: Use of `global` in async context without explicit locking.")
