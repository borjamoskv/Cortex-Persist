...
async def deprecate_impl_logic(
    ...
    """Implement the core logic for fact deprecation."""
    ...
    try:
        ...
        logger.warning("FTS cleanup failed during deprecation for fact %s: %s", fact_id, e)
...
async def invalidate_impl_logic(
    ...
    """Implement the core logic for fact invalidation."""
    from cortex.engine.causality import AsyncCausalGraph
    from cortex.engine.mutation_engine import MUTATION_ENGINE
    ...
    try:
        ...
        logger.warning("FTS cleanup failed during invalidation for fact %s: %s", fact_id, e)
...
async def purge_logic(mixin_instance: Any, fact_id: int, tenant_id: str, force: bool) -> bool:
    """Implement the core logic for fact purging."""
    async with mixin_instance.session() as conn:
        ...
        criticality = Decimal("0.5") if fact_type == "rule" else Decimal("0.0")
        criticality += min(Decimal("0.4"), Decimal(dep_count) * Decimal("0.1"))
        ...
        if not decision.allowed and not force:
            if decision.requires_quarantine:
                await mixin_instance.invalidate(
                    fact_id,
                    reason=f"Quarantined: {decision.reason}",
                    conn=conn,
                    tenant_id=tenant_id,
                )
