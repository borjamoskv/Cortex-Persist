...
class NarrativeCompiler:
    ...
    def _build_vector_field(self, trajectories: List[MemoryTrajectory]) -> List[NarrativeVector]:
        ...
        for i, t in enumerate(trajectories):
            ...
            tension = float(np.var([p.probability_mass for p in t.particles])) if len(t.particles) > 1 else 0.0
    ...
    def _energy_filter(self, field: List[NarrativeVector], budget_threshold: float) -> List[NarrativeVector]:
    ...
    def _collapse_direct(self, field: List[NarrativeVector]) -> ActionOutput:
        ...
        """
        if not field:
            return ActionOutput(type="DIRECT_ACTION", content="NO_ACTION", confidence=0.0, energy_cost=0.0)
    ...
    def _guided_decision(self, field: List[NarrativeVector]) -> ActionOutput:
        ...
        # Sort by attractor
        sorted_field = sorted(field, key=lambda nv: (nv.phase_alignment - nv.energy_signature), reverse=True)
        top_options = sorted_field[:min(5, len(sorted_field))]
        
        content = [{"trajectory": nv.trajectory_id, "weight": nv.phase_alignment} for nv in top_options]
    ...
    def compile(self, simulation_field: SimulationField, thermo_state: ThermodynamicState) -> ActionOutput:
