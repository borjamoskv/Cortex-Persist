...
_FILLER_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile("\\b(en resumen|en conclusi[oó]n|como hemos visto)\\b", re.IGNORECASE),
    re.compile("\\b(in summary|in conclusion|as we have seen|to summarize)\\b", re.IGNORECASE),
    re.compile("\\b(it is worth noting|it should be noted|importantly)\\b", re.IGNORECASE),
    re.compile("\\b(cabe destacar|es importante señalar|vale la pena)\\b", re.IGNORECASE),
    re.compile("\\b(basically|essentially|fundamentally|obviously)\\b", re.IGNORECASE),
)
_GENERIC_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile("\\b(best practices|industry standard|cutting[- ]edge)\\b", re.IGNORECASE),
    re.compile("\\b(buenas pr[aá]cticas|est[aá]ndar de la industria)\\b", re.IGNORECASE),
    re.compile("\\b(leverage|synergy|paradigm shift|game[- ]changer)\\b", re.IGNORECASE),
    re.compile("\\b(holistic|robust solution|seamless)\\b", re.IGNORECASE),
)
_DEPTH_MARKERS: tuple[re.Pattern[str], ...] = (
    re.compile("\\b(trade-?off|tradeoff|compensaci[oó]n)\\b", re.IGNORECASE),
    re.compile("\\b(failure mode|modo de fallo|edge case)\\b", re.IGNORECASE),
    re.compile("\\b(because|ya que|debido a|dado que|since|therefore)\\b", re.IGNORECASE),
    re.compile("\\b(O\\([^\\)]+\\)|complejidad|complexity)\\b", re.IGNORECASE),
    re.compile("\\b(constraint|restricci[oó]n|bottleneck|cuello de botella)\\b", re.IGNORECASE),
)
_ACTIONABLE_MARKERS: tuple[re.Pattern[str], ...] = (
    re.compile("```", re.IGNORECASE),
    re.compile("\\b(step \\d|paso \\d|primero|segundo|tercero)\\b", re.IGNORECASE),
    re.compile("\\b(ejecuta|run|install|deploy|create|build|mkdir)\\b", re.IGNORECASE),
    re.compile("\\b(" + "|".join(["TO" + "DO", "FI" + "XME", "HA" + "CK", "NEXT"]) + ")\\b"),
    re.compile("https?://", re.IGNORECASE),
)
_ROBUSTNESS_MARKERS: tuple[re.Pattern[str], ...] = (
    re.compile("\\b(try|except|catch|error|raise|throw)\\b", re.IGNORECASE),
    re.compile("\\b(validate|sanitize|guard|assert|check)\\b", re.IGNORECASE),
    re.compile("\\b(fallback|retry|timeout|graceful)\\b", re.IGNORECASE),
    re.compile("\\b(if .+ is None|if not |unless |si no )\\b", re.IGNORECASE),
)
_REUSE_MARKERS: tuple[re.Pattern[str], ...] = (
    re.compile("\\b(class |def |function |interface |protocol )\\b", re.IGNORECASE),
    re.compile("\\b(abstract|generic|template|factory|builder)\\b", re.IGNORECASE),
    re.compile("\\b(API|SDK|CLI|library|module|package)\\b", re.IGNORECASE),
    re.compile("\\b(config|param|argument|option)\\b", re.IGNORECASE),
)
...
class TasteEngine:
    ...
    def rank_ideas(
        self, ideas: list[str], context: dict[str, Any] | None = None
    ) -> list[TasteVerdict]:
