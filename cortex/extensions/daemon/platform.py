...
def install_macos() -> None:
    if not PLIST_SOURCE.exists():
        console.logger.info(f"[red]❌ Plist not found: {PLIST_SOURCE}[/]")
        sys.exit(1)
    dest = _get_plist_dest()
    dest.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(PLIST_SOURCE, dest)
    console.logger.info(f"[green]✅ Installed:[/] {dest}")
    import subprocess

    subprocess.run(["launchctl", "load", str(dest)], check=False)
    console.logger.info(f"[green]✅ Loaded:[/] {BUNDLE_ID}")
    console.logger.info("[dim]   Daemon will run every 5 minutes and on login.[/]")
...
def uninstall_macos() -> None:
    ...
    if dest.exists():
        ...
        dest.unlink()
        console.logger.info(f"[green]✅ Removed:[/] {BUNDLE_ID}")
    else:
        console.logger.info("[yellow]No launchd agent installed.[/]")
...
"""
    ...
    console.logger.info(f"[green]✅ Installed:[/] {unit_path}")
    import subprocess

    subprocess.run(["systemctl", "--user", "daemon-reload"], check=False)
    subprocess.run(["systemctl", "--user", "enable", "--now", BUNDLE_ID], check=False)
    console.logger.info(f"[green]✅ Enabled:[/] {BUNDLE_ID}")
    console.logger.info("[dim]   Daemon will run automatically on login.[/]")
...
def uninstall_linux() -> None:
    ...
    if unit_path.exists():
        ...
        console.logger.info("[yellow]No systemd unit installed.[/]")
...
def install_windows() -> None:
    ...
    if result.returncode == 0:
        console.logger.info(f"[green]✅ Installed:[/] Task Scheduler → {task_name}")
        console.logger.info("[dim]   Daemon will run automatically on login.[/]")
    else:
        console.logger.info(f"[red]❌ Failed:[/] {result.stderr.strip()}")
...
def uninstall_windows() -> None:
    ...
    if result.returncode == 0:
        console.logger.info(f"[green]✅ Removed:[/] {task_name}")
    else:
        console.logger.info("[yellow]No scheduled task found.[/]")
