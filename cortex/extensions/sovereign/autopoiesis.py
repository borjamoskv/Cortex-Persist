...
def _atomic_write_text(path: Path, content: str) -> None:
    """Persist a generated tool script atomically."""
    path.parent.mkdir(parents=True, exist_ok=True)
    fd, temp_path = tempfile.mkstemp(prefix="autopoiesis_", dir=str(path.parent), text=True)
    try:
...
class Autopoiesis:
    ...
    def generate_and_register(
        ...
        _atomic_write_text(script_path, source)
        logger.debug("Manifesting tool: %s", script_name)

        result = self._execute_sandbox(script_path)
        if result.returncode != 0:
            logger.error("Autopoietic manifestation failed: %s", result.stderr)
        ...
        logger.info("Tool registered: %s", script_name)
        return script_path
