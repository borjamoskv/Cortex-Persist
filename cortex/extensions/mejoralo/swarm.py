...
class MejoraloSwarm:
    ...
    async def refactor_file(self, file_path: Path, findings: list[str], iteration: int=0, engine: MejoraloEngine | None=None, project: str | None=None) -> str | None:
        ...
        logger.info(f'  [dim]🐝 Swarm (L{self.level}) pensando en {file_path.name}...[/]')
        if file_path.suffix == '.py':
            result = await self._surgical_refactor(file_path, content, findings, findings_str, scars_str, swarm_system)
            if result is not None:
                logger.info(f'  [green]✨ Cirujía AST completada [{file_path.name}][/]')
                return result
            logger.info('  [yellow]⚠️ Modo quirúrgico fallido — fallback a archivo completo.[/]')
        ...
        if result_content:
            logger.info(f'  [green]✨ Síntesis completada para {file_path.name}[/]')
