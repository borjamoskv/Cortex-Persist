...
def score_semantics(files: list[Path]) -> tuple[float, list]:
    ...
    if not files:
        return (0.0, [])
    from cortex.extensions.moltbook.skills.sovereign_code_scorer.scripts.sovereign_scorer import Issue  # pyright: ignore[reportMissingImports]
    issues: list[Issue] = []
    total_score = 25.0
    deductions = 0.0
    ...
    for f in py_files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Name) and len(node.id) == 1 and (node.id not in _ALLOWED_SHORT):
                deductions += 0.2
                issues.append(Issue(file=str(f), line=node.lineno, category='semantics', severity='info', message=f"Single-char variable name '{node.id}'"))
        for node in ast.walk(tree):
            ...
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                if not (node.body and isinstance(node.body[0], ast.Expr) and isinstance(node.body[0].value, ast.Constant)):
                    if not node.name.startswith('_'):
                        deductions += 0.3
                        if len(issues) < 50:
                            issues.append(Issue(file=str(f), line=node.lineno, category='semantics', severity='info', message=f"Function '{node.name}' lacks docstring"))
...
def score_tests(root: Path, files: list[Path]) -> tuple[float, list]:
    ...
    if test_ratio == 0:
        issues.append(Issue(file='project', line=0, category='tests', severity='critical', message='No test files found. Testing is not optional.'))
        return (0.0, issues)
    if test_ratio >= 0.8:
        ratio_score = 10.0
    elif test_ratio >= 0.5:
