    ...
    def _visit_nested(self, node):
        ...
        if self._current_function:
            self.nesting[self._current_function] = max(
                self.nesting.get(self._current_function, 0), 
                self._current_nesting
            )
        self.generic_visit(node)
        self._current_nesting -= 1
...
class OuroborosOmega:
    ...
    def __init__(self, target_path: str, project_root: str = "~/.gemini/antigravity/scratch/Cortex-Persist", dry_run: bool = False):
    ...
    async def diagnose(self, source_code: str | None = None) -> DiagnosisMatrix:
        ...
        dead_funcs = {f for f, callers in called_by.items() if not callers and f.startswith("_") and f != "__init__"}
        ...
        entropy = min(100.0, loc_penalty + complexity_penalty + dead_code_ratio + unused_imports_penalty + nesting_penalty)
    ...
    async def execute_atomic_cycle(self) -> dict[str, Any]:
        ...
        try:
            ...
            # 2. EXTRACTION
            purger = _DeadCodePurge(base_diagnosis.dead_interfaces, base_diagnosis.imports - base_diagnosis.used_imports)
            ...
            if base_diagnosis.blocking_calls:
                logger.warning("Phase 4 [Scaling]: Detected blocking I/O: %s", base_diagnosis.blocking_calls)
            ...
            if entropy_delta > 5.0:
                logger.warning("Phase 5 [Verification] Entropy increased significantly (+%.2f). APOPTOSIS.", entropy_delta)
                return {"status": "ROLLED_BACK", "reason": f"Entropy regression: {entropy_delta:.2f}"}
