...
logger = logging.getLogger("inject_cap_theorem")
...
async def inject_primitives():
    ...
    md_path = "AUTODIDACT_CAP_THEOREM.md"
    with open(md_path, encoding="utf-8") as f:
        content = f.read()

    pattern = re.compile(r'-\s*\*\*(CAP-\d{3})\*\*:\s*`?([^`\n]+?)`?\s*-\s*(.*)')
    matches = pattern.findall(content)

    agent_id = 99
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "CAP_THEOREM_ROOT", 0.0)
    ...
    for i, match in enumerate(matches):
        ...
        ultramap.update_agent_position(agent_id, (i + 1) * 1.0, 0.0, 0.0, "CAP_THEOREM_LEAF", 0.1)
