...
def generate_drift():
    ...
    for i in range(1000):
        if i >= 700:
            drift = (i - 700) / 300.0
            data.append(np.random.normal(loc=[5.0, 0.01, 0.2 + 0.6*drift, 0.1 + 0.8*drift], scale=[0.5, 0.001, 0.02, 0.02]))
...
def evaluate_detector(detector_cls, kwargs, data, y_true):
    ...
    tn, fp, fn, tp = confusion_matrix(y_eval, p_eval).ravel() if len(np.unique(y_eval)) > 1 else (len(y_eval),0,0,0)
    fpr = fp / max(1, (fp + tn))
...
def main():
    ...
    configs = {
        "Original (d=1)": {"window_size": 100, "anomaly_threshold": 3.0, "takens_dim": 1, "takens_tau": 1},
        "Takens (d=3, t=2)": {"window_size": 100, "anomaly_threshold": 3.0, "takens_dim": 3, "takens_tau": 2}
    }
    ...
    for s_name in scenarios:
        for c_name in configs:
            ...
            print(f"| {s_name} | {c_name} | {m['AUC']:.4f} | {m['Precision']:.4f} | {m['Recall']:.4f} | {m['FPR']:.4f} | {m['Throughput']:,.0f} |")
