...
# Protocolo: C5-REAL | Inyección de Primitivas de la Teoría Unificada de Computación Semántica
...
logger = logging.getLogger("inject_teoria_unificada")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas TEORÍA UNIFICADA en CORTEX-Persist (C5-REAL)...")
    ...
    md_path = "AUTODIDACT_TEORIA_UNIFICADA.md"
    ...
    pattern = re.compile(r'-\s*\*\*(TEORIA-UNIFICADA-\d{3})\*\*:\s*`?([^`]+)`?\s*-\s*(.*)')
    ...
    agent_id = 99  # Agent ID reservado para TEORIA-UNIFICADA INJECTOR
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "TEORIA_UNIFICADA_ROOT", 0.0)
    ...
    for match in matches:
        ...
        try:
            ...
            ultramap.update_agent_position(agent_id, injected_count * 1.0, 0.0, 0.0, "TEORIA_UNIFICADA_LEAF", 0.1)
