...
| Surface | Name | Notes |
|:---|:---|:---|
| **PyPI package** | `babylon60` | `pip install babylon60` |
...
| **JS/TS SDK** | `@babylon60/sdk` | Not yet published on npm — roadmap item |

---

## Rationale

### PyPI Package: `babylon60`

The root `pyproject.toml` defines the canonical PyPI distribution name as `babylon60`. This is the authoritative package. The `cortex-sdk/` and `sdks/python/` directories are **separate, experimental thin-client SDKs** for the hosted cloud API — they are not the same package and are not yet published.

### Python Import: `cortex`

The Python source tree lives under the `cortex/` directory at the root of the repository. When users install `pip install babylon60`, they import from `cortex`:
...
### JS/TS SDK: `@babylon60/sdk`

The JavaScript/TypeScript SDK lives in `sdks/js/` and will eventually be published as `@babylon60/sdk` on npm.

> ⏳ **Roadmap.** Not yet published. `npm install @babylon60/sdk` returns 404 until the first npm release.
...
| `pip install cortex_persist` | `pip install babylon60` | Underscore is wrong |
| `npm install @babylon60/sdk` (without caveat) | Add "⏳ Coming Soon" note | Not yet on npm |
