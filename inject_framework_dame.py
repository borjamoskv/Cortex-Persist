...
# Protocolo: C5-REAL | Inyección del Framework DAME
# ID de Agente asignado: 108
...
logger = logging.getLogger("inject_framework_dame")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas DAME en CORTEX-Persist (C5-REAL)...")
    ...
    md_path = "AUTODIDACT_FRAMEWORK_DAME.md"
    ...
    agent_id = 108  # Agent ID reservado para DAME FRAMEWORK INJECTOR
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "DAME_ROOT", 0.0)
    ...
    for match in matches:
        ...
        try:
            ...
            ultramap.update_agent_position(agent_id, injected_count * 1.0, 0.0, 0.0, "DAME_LEAF", 0.1)
