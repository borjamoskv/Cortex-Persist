...
logger = logging.getLogger("inject_git_exergy")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas GIT-EXERGY en CORTEX-Persist (C5-REAL)...")
    ...
    md_path = "AUTODIDACT_GIT_EXERGY_AI.md"
    ...
    # Formato línea: - **GIT-EXERGY-001**: `git init` - Instanciación de Estado Cero
    pattern = re.compile(r'-\s*\*\*(GIT-EXERGY-\d{3})\*\*:\s*`?([^`\n]+?)`?\s*-\s*(.*)')
    ...
    agent_id = 98  # Agent ID reservado para GIT-EXERGY INJECTOR
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "GIT_ROOT", 0.0)
    ...
    for match in matches:
        ...
        try:
            ...
            ultramap.update_agent_position(agent_id, injected_count * 1.0, 0.0, 0.0, "GIT_LEAF", 0.1)
