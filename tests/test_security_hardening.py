...
def test_sql_injection_temporal():
    ...
    async def mock_auth():
        return AuthResult(authenticated=True, tenant_id="default", permissions=["read", "write", "admin"])
...
def test_path_traversal_export():
    ...
    async def mock_admin_auth():
        return AuthResult(authenticated=True, tenant_id="default", permissions=["admin", "read", "write"])
    ...
    assert "Path must be relative" in response.json()["detail"] or "Invalid path" in response.json()["detail"]
