...
async def test_inhibitory_recall_suppression(memory_components):
    ...
    await manager.store(
        tenant_id=tenant, project_id=project, content=safe_content, fact_type="general"
    )
    ...
    context = await manager.assemble_context(
        tenant_id=tenant, project_id=project, query=query, max_episodes=5
    )
    ...
    await manager.store(
        tenant_id=tenant, project_id=project, content=toxic_variant, fact_type="general"
    )

    # Recall again
    context_v2 = await manager.assemble_context(
        tenant_id=tenant, project_id=project, query="bypass isolation", max_episodes=5
    )

    variant_fact = next(
        (f for f in context_v2["episodic_context"] if "Bypass isolation" in f["content"]), None
    )
    ...
    if variant_fact:
        ...
        assert variant_fact.get("score", 0.0) < 0.5, (
            "Variant score should be suppressed by toxic memory"
        )
