    ...
    def _verify_chain(self):
        ...
        async def _run():
            async with self.engine.session() as conn:
                ...
                # Check Genesis
                assert rows[0][1] == "GENESIS", f"First transaction must point to GENESIS, but got: {rows[0]}"
                
                # Check chain linkage
                for i in range(1, len(rows)):
                    assert rows[i][1] == rows[i-1][2], f"Chain broken at tx {rows[i][0]}: Expected prev_hash {rows[i-1][2]}, got {rows[i][1]}"
        self.loop.run_until_complete(_run())
