...
from babylon60.extensions.fingerprint.models import CognitiveFingerprint
...
def _patch_scanner(mock_data: dict):
    ...
    scanner_patch = patch(
        "babylon60.extensions.fingerprint.extractor.FingerprintScanner", autospec=True
    )
...
class TestFingerprintEmpty:
    ...
    async def test_empty_db_returns_null_state(self):
        engine = _make_engine()
        with patch("babylon60.extensions.fingerprint.extractor.FingerprintScanner") as MockScanner:
            m = AsyncMock()
