...
def test_logout_no_changes(runner):
    ...
    with patch("cortex.cli.session_cmds._get_uncommitted_changes", return_value=[]), \
...
def test_logout_with_changes_and_recent_decision(runner):
    ...
    with patch("cortex.cli.session_cmds._get_uncommitted_changes", return_value=["M file.py"]), \
