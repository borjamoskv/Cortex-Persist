...
async def test_siege_verification_instruction_injection(mock_queue):
    ...
    with (
        patch("cortex_extensions.aether.runner.PlannerAgent", return_value=planner_mock),
        patch("cortex_extensions.aether.runner.ExecutorAgent", return_value=executor_mock),
        patch("cortex_extensions.llm.provider.LLMProvider") as mock_llm_cls,
        patch("cortex_extensions.aether.runner.AgentToolkit") as mock_toolkit,
    ):
