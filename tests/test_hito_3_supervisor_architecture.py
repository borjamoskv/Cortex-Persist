...
from cortex.engine.swarm.legion import SwarmAgent, SwarmSignal
...
async def test_ghost_state_recovery():
    ...
    verify_db = await aiosqlite.connect(db_path)
    async with verify_db.execute("SELECT count(*) FROM system_hypotheses WHERE status = 'ACTIVE'") as cur:
