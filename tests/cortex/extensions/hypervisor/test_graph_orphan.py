...
class MockEngine:
    ...
    async def store(self, content, fact_type, project, source, meta, confidence):
        ...
        self.facts = [
            f for f in self.facts if f.meta.get("belief_object", {}).get("id") != belief_id
        ]
...
async def test_cascading_quarantine_graph_orphan():
    ...
    assert active_count == total_beliefs, (
        f"Expected {total_beliefs} active beliefs, got {active_count}"
    )
    ...
    active_count_post = sum(
        1 for f in mock_db.facts if f.meta["belief_object"]["status"] == "active"
    )
    quarantined_count = sum(
        1 for f in mock_db.facts if f.meta["belief_object"]["status"] == "quarantined"
    )

    assert active_count_post == 0, (
        f"Expected 0 active beliefs post-collapse, got {active_count_post}"
    )
    assert quarantined_count == total_beliefs, (
        f"Expected {total_beliefs} quarantined beliefs, got {quarantined_count}"
    )
