...
def manifest():
    return AgentManifest(
        agent_id="TUTOR-META",
        purpose="x10 Governance enforcing the Eight Laws"
    )
...
async def test_tutor_deadlock_annihilation_warning(manifest, mock_bus):
    ...
    mock_resolve.return_value = MagicMock(
        resolution=MagicMock(method="DEADLOCK_HEURISTIC"),
        to_dict=lambda: {"status": "resolved"}
    )
    tutor.conflict_resolver.resolve = mock_resolve
    
    agents_data = {
        "agent-A": {"specialty": "general"},
        "agent-B": {"specialty": "general"}
    }
    options_data = [
