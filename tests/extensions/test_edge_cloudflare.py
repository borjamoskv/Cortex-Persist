...
async def test_cloudflare_edge_bridge_sync():
    ...
    result = await bridge.sync_ledger_to_edge(
        taint="ZK-001", payload_hash="abcd123", payload="data"
    )
...
async def test_cloudflare_edge_bridge_real_sync():
    ...
    bridge = CloudflareEdgeBridge(
        account_id="test_acc", api_token="test_token", database_id="db123"
    )
    assert (
        bridge.base_url
        == "https://api.cloudflare.com/client/v4/accounts/test_acc/d1/database/db123/query"
    )
