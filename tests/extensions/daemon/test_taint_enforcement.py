...
from cortex.engine.causal.verification_oracle import verify_c5_state_machine, InvariantViolationError
...
def agent_keys():
    ...
    priv_b64 = base64.b64encode(priv.private_bytes(Encoding.Raw, PrivateFormat.Raw, NoEncryption())).decode("ascii")
    pub_b64 = base64.b64encode(priv.public_key().public_bytes(Encoding.Raw, PublicFormat.Raw)).decode("ascii")
...
def clean_db():
    ...
    """)
    conn.execute("CREATE TABLE IF NOT EXISTS agents (id TEXT PRIMARY KEY, name TEXT, agent_type TEXT, reputation_score REAL, public_key TEXT, tenant_id TEXT, is_active INTEGER DEFAULT 1)")
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS auth_requests (id TEXT PRIMARY KEY, hypothesis TEXT, state_payload TEXT, status TEXT, created_at REAL, resolved_at REAL, signature TEXT, public_key TEXT)")
...
async def setup_aio_db(aio_conn, agent_id, pub_key):
    ...
    """)
    await aio_conn.execute("CREATE TABLE IF NOT EXISTS agents (id TEXT PRIMARY KEY, name TEXT, agent_type TEXT, reputation_score REAL, public_key TEXT, tenant_id TEXT, is_active INTEGER DEFAULT 1)")
    await aio_conn.execute("CREATE TABLE IF NOT EXISTS enrichment_jobs (fact_id INTEGER, job_type TEXT, status TEXT, priority INTEGER)")
    await aio_conn.execute("CREATE TABLE IF NOT EXISTS fact_tags (fact_id INTEGER, tag TEXT, tenant_id TEXT)")
    await aio_conn.execute("CREATE TABLE IF NOT EXISTS facts_fts (rowid INTEGER, content TEXT, project TEXT, tags TEXT, fact_type TEXT, tenant_id TEXT)")
    await aio_conn.execute("CREATE TABLE IF NOT EXISTS causal_edges (fact_id INTEGER, parent_id INTEGER, signal_id TEXT, edge_type TEXT, project TEXT, tenant_id TEXT)")
    await aio_conn.execute("CREATE TABLE IF NOT EXISTS auth_requests (id TEXT PRIMARY KEY, hypothesis TEXT, state_payload TEXT, status TEXT, created_at REAL, resolved_at REAL, signature TEXT, public_key TEXT)")
    await aio_conn.execute("CREATE TABLE IF NOT EXISTS taint_nonces (nonce TEXT PRIMARY KEY, timestamp REAL)")
...
async def test_hdc_store_l2_taint_rejection(tmp_path, agent_keys):
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS agents (id TEXT PRIMARY KEY, name TEXT, agent_type TEXT, reputation_score REAL, public_key TEXT, tenant_id TEXT, is_active INTEGER DEFAULT 1)")
    conn.execute("INSERT INTO agents (id, name, agent_type, reputation_score, public_key, tenant_id) VALUES (?, ?, ?, 1.0, ?, 'default')", (agent_id, "Agent", "ai", pub))
    conn.commit()
...
async def test_write_trait_taint_rejection(clean_db, agent_keys):
    ...
    clean_db.execute("INSERT INTO agents (id, name, agent_type, reputation_score, public_key, tenant_id) VALUES (?, ?, ?, 1.0, ?, 'default')", (agent_id, "Agent", "ai", pub))
    clean_db.commit()
...
async def test_verification_oracle_invariants(agent_keys):
    ...
    async with aiosqlite.connect(":memory:") as aio_conn:
        ...
        await aio_conn.execute("INSERT INTO causal_edges (fact_id, parent_id, edge_type, project, tenant_id) VALUES (1, NULL, 'trigger', 'p1', 'default')")
        await aio_conn.execute("INSERT INTO causal_edges (fact_id, parent_id, edge_type, project, tenant_id) VALUES (2, 1, 'trigger', 'p1', 'default')")
        await aio_conn.commit()
        assert await verify_c5_state_machine(aio_conn) is True
        
        # 3. Add a cycle to causal_edges (violates Inv-3)
        await aio_conn.execute("INSERT INTO causal_edges (fact_id, parent_id, edge_type, project, tenant_id) VALUES (1, 2, 'trigger', 'p1', 'default')")
        await aio_conn.commit()
        ...
        await aio_conn.execute("INSERT INTO auth_requests (id, hypothesis, state_payload, status) VALUES ('req_1', 'hyp', '{}', 'APPROVED')")
        await aio_conn.commit()
        with pytest.raises(InvariantViolationError):
            await verify_c5_state_machine(aio_conn)
