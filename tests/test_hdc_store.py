...
def hdc_store(tmp_path):
    ...
    store = HDCVectorStoreL2(encoder=enc, item_memory=mem, db_path=db_path, half_life_days=7)
    return store
...
async def test_extract_traces(hdc_store):
    ...
    results = await hdc_store.recall_secure(
        tenant_id="test_tenant", project_id="alpha-project", query="binding XOR inversion", limit=1
    )
