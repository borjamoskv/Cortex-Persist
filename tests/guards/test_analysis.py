...
def test_scan_args_for_oracles_rejection():
    ...
    assert set(found) == {
        "gpt",
        "claude",
    }  # ollama variable doesn't have an oracle string constant, but variable name checking?
...
def test_scan_list_rejection():
    tree = ast.parse("['gpt', ollama_var]")
    assert set(scan_list(tree.body[0].value)) == {"gpt", "ollama_var"}
