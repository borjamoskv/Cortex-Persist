...
def test_store_with_chronos(tmp_path):
    ...
    # Verify the fact via the engine to handle decryption automatically
    facts = engine.recall_sync("test_project")
    assert facts, "Fact not found in database"
    fact = facts[0]

    assert fact.meta is not None
    assert "chronos" in fact.meta
    chronos_meta = fact.meta["chronos"]
