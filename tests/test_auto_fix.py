...
def test_classify_ghosts():
    ...
    assert AutoFixPipeline.classify("FIX" + "ME: memory leak here") == GhostClass.DOC_GAP
...
@pytest.mark.asyncio
@patch("cortex_extensions.swarm.auto_fix.AutoFixPipeline._execute")
@patch("cortex_extensions.swarm.auto_fix.AutoFixPipeline._escalate")
...
async def test_process_ghost_unknown_classification():
    ...
    with patch("cortex_extensions.swarm.auto_fix.AutoFixPipeline._execute") as mock_exe:
        result = await pipeline.process_ghost(ghost)
