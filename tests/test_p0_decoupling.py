...
async def test_store_decoupled(engine):
    ...
    async with aiosqlite.connect(str(engine._db_path)) as db:
        async with db.execute("SELECT fact_id, status FROM enrichment_jobs WHERE fact_id = ?", (fact_id,)) as cursor:
...
async def test_worker_processing(engine):
    ...
    # Store a fact
    fact_id = await engine.store(project="test_project", content="Job processing test", source="worker_test")
    ...
    # Process one job
    async with aiosqlite.connect(str(engine._db_path)) as db:
        async with db.execute("SELECT id, fact_id FROM enrichment_jobs WHERE fact_id = ?", (fact_id,)) as cursor:
    ...
    async with aiosqlite.connect(str(engine._db_path)) as db:
        async with db.execute("SELECT status FROM enrichment_jobs WHERE fact_id = ?", (fact_id,)) as cursor:
            row = await cursor.fetchone()
