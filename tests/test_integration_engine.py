...
async def test_connection_pool_stability(engine):
    ...
    async with engine.session() as conn:
        with causal_write(conn):
            await conn.execute(
                "INSERT INTO facts (id, content, project, metadata) VALUES (1, 'Test Fact', 'test_proj', '{\"cortex_taint\": \"trusted\"}')"
            )
            for i in range(100):
                await conn.execute(
...
async def test_multi_tenant_isolation(engine):
    ...
    async with engine.session() as conn:
        with causal_write(conn):
            ...
            await conn.execute(
                "INSERT INTO facts (id, content, project, metadata) VALUES (10, 'Tenant Fact', 'test_proj', '{\"cortex_taint\": \"trusted\"}')"
            )
...
async def test_taint_propagation_crypto_safety(engine):
    ...
    async with engine.session() as conn:
        with causal_write(conn):
            # 1. Insert fact 500
            encrypted_blob = '{"cortex_taint": "trusted", "blob": "Ω-ENCRYPTED-CRYPTO-STUFF-Ω"}'
            await conn.execute(
            ...
            await conn.execute(
                "INSERT INTO facts (id, content, project, confidence, metadata) VALUES (?, ?, ?, ?, ?)",
                (501, "Derived Fact", "project_x", "C5", '{"cortex_taint": "trusted"}'),
            )
