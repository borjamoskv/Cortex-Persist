...
def test_cortex_arithmetic():
    a = cortex_rs.Cortex(SCALE) # 1.0
    b = cortex_rs.Cortex(int(SCALE / 2)) # 0.5
...
def test_cortex_comparisons():
    a = cortex_rs.Cortex(SCALE) # 1.0
    b = cortex_rs.Cortex(SCALE * 2) # 2.0
    c = cortex_rs.Cortex(SCALE) # 1.0
...
def test_cortex_hash():
    a = cortex_rs.Cortex(SCALE)
    b = cortex_rs.Cortex(SCALE)
    c = cortex_rs.Cortex(SCALE * 2)
    
    s = {a, c}
    assert b in s
    assert c in s

def test_cortex_from_int():
    a = cortex_rs.Cortex.from_int(5)
