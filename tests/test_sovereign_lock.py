...
async def test_sovereign_lock_priority(engine):
    ...
    cursor = await engine.session().execute("SELECT holder_agent FROM lock_state WHERE resource = 'res'")
    ...
    assert (await (await engine.session().execute("SELECT holder_agent FROM lock_state WHERE resource = 'res'")).fetchone())[0] == "agent_B"
