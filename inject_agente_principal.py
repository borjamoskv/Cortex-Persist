...
logger = logging.getLogger("inject_agente_principal")

async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas AGENT-PRINCIPAL en CORTEX-Persist (C5-REAL)...")
    ...
    md_path = "AUTODIDACT_AGENTE_PRINCIPAL.md"
    ...
    agent_id = 99  # Agent ID reservado para AGENT-PRINCIPAL INJECTOR
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "AGENT_PRINCIPAL_ROOT", 0.0)
    ...
    for match in matches:
        ...
        try:
            ...
            ultramap.update_agent_position(agent_id, injected_count * 1.0, 0.0, 0.0, "AGENT_PRINCIPAL_LEAF", 0.1)
