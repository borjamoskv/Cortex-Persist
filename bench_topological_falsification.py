...
def run_advanced_suite():
    ...
    detector_drift = WindowedManifoldDetector(
        window_size=100, anomaly_threshold=3.0, use_velocity=True
    )
    drift_anomalies = 0
    ...
    logger.info(
        f"Velocity Embedding assimilation: {'SUCCESS' if drift_anomalies < 10 else 'FAILED'}"
    )
    ...
    d = 64
    detector_dim = WindowedManifoldDetector(
        window_size=200, anomaly_threshold=5.0, use_velocity=False
    )
    # Warmup
    for _ in range(200):
    ...
    logger.info(
        f"Matrix inversion stable at d={d}: {not np.isnan(res_shock['mahalanobis_distance'])}"
    )
    logger.info(
        f"Shock detected in R^{d}: {res_shock['is_anomaly']} (Dist: {res_shock['mahalanobis_distance']:.2f})"
    )
