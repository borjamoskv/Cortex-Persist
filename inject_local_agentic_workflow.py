...
async def inject_primitives():
    logger.info("Iniciando inyección de Primitivas para LOCAL-AGENTIC-WORKFLOW...")
    ...
    md_path = "AUTODIDACT_LOCAL_AGENTIC_WORKFLOW.md"
    with open(md_path, encoding="utf-8") as f:
        content = f.read()

    pattern = re.compile(r'-\s*\*\*(HERMES-LOCAL-\d{3})\*\*:\s*`?([^`]+)`?\s*-\s*(.*)')
    ...
    agent_id = 101  # Asignar un ID de agente único para esta inyección
    ultramap.update_agent_position(agent_id, 0.0, 0.0, 0.0, "LOCAL-AGENTIC-WORKFLOW_ROOT", 0.0)
    ...
    for i, match in enumerate(matches):
        ...
        ultramap.update_agent_position(agent_id, (i + 1) * 1.0, 0.0, 0.0, "LOCAL-AGENTIC-WORKFLOW_LEAF", 0.1)
