...
def run_adversarial_swarm(case_id: str, case_data: Dict[str, Any]):
    ...
    append_event(case_id, "PHASE_START", {"phase": "IR_GRAPH_GENERATION", "status": "Building Abstract Syntax Tree & CFG"})
    time.sleep(1)
    
    append_event(case_id, "PHASE_START", {"phase": "ATTACK_SURFACE_MAP", "status": "Mapping entrypoints and public methods"})
    time.sleep(1)
    
    append_event(case_id, "SWARM_ACTIVATION", {"agents": ["reentrancy_hunter", "auth_bypass_simulator", "invariant_breaker"]})
    ...
    # 1. Reentrancy
    append_event(case_id, "AGENT_HYPOTHESIS", {"agent": "reentrancy_hunter", "target": "withdraw()", "hypothesis": "State update after external call"})
    if case_data.get("contract_address"):
        ...
        append_event(case_id, "AGENT_SUCCESS", {"agent": "reentrancy_hunter", "finding": "Reentrancy confirmed on withdraw()"})
    else:
        append_event(case_id, "AGENT_NULL", {"agent": "reentrancy_hunter", "finding": "Null result"})
        
    time.sleep(1)
    
    # 2. Auth Bypass
    append_event(case_id, "AGENT_HYPOTHESIS", {"agent": "auth_bypass_simulator", "target": "admin_panel", "hypothesis": "Missing role check"})
    findings.append({
        "type": "auth_bypass",
        "severity": 8.5,
    ...
    append_event(case_id, "AGENT_SUCCESS", {"agent": "auth_bypass_simulator", "finding": "Auth bypass confirmed via forged token"})
    ...
    append_event(case_id, "LEDGER_COMMIT", {"merkle_root": ledger_entry["commit"], "signature": signature})
