    ...
    async with aiosqlite.connect(db_path, timeout=10) as conn:
        ...
        await conn.execute(
            ...
            (
                ...
                "parasite_void",
                0.0,
                json.dumps(["parent_that_never_existed"]),
                "crystallized",
                True,
            ),
        )
        ...
        await conn.execute(
            "UPDATE thermodynamics_state SET entropy_budget = entropy_budget + 1000 WHERE tenant_id = ?",
            (tenant_id,),
        )
        await conn.commit()
...
async def parasite_test():
    ...
    for s in current_multiverse:
        ...
        print(
            f"[{status}] {s['tenant_id']} | CF: {s['cf']:.2f} | EB: {s['eb']:.1f} | Exergy: {s['exergy']:.1f}"
        )
